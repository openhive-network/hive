...
inline void unpack( Stream& s, hive::protocol::asset_symbol_type& sym, uint32_t )
{
  ...
  switch( ser )
  {
    ...
    case (uint64_t(3) | ((uint64_t('S') | (uint64_t('T') << 8) | (uint64_t('E') << 16) | (uint64_t('E') << 24) | (uint64_t('M') << 32)) << 8)) & 0xFFFFFFFF:
      s.read( ((char*) &ser)+4, 4 );
      FC_ASSERT( ser == (uint64_t(3) | ((uint64_t('S') | (uint64_t('T') << 8) | (uint64_t('E') << 16) | (uint64_t('E') << 24) | (uint64_t('M') << 32)) << 8)), "invalid asset bits" );
      sym.asset_num = HIVE_ASSET_NUM_HIVE;
      break;
    case (uint64_t(3) | ((uint64_t('S') | (uint64_t('B') << 8) | (uint64_t('D') << 16)) << 8)) & 0xFFFFFFFF:
      s.read( ((char*) &ser)+4, 4 );
