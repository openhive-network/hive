...
{
  HIVE_PROTOCOL_STRING_ASSERT( permlink.size() < HIVE_MAX_PERMLINK_LENGTH, "permlink is too long: `${subject}`", ("subject", permlink) );
  HIVE_PROTOCOL_STRING_ASSERT( fc::is_utf8( permlink ), "permlink not formatted in UTF8", ("subject", permlink) );
}
