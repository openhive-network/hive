...
class sign_state
{
  ...
  public:
    ...
    bool check_authority( const string& id )
    {
      ...
      if constexpr (IS_TRACED) {
        HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 1");
        try
        {
      ...
      {
        if constexpr (IS_TRACED) {
          HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 2");
      ...
      if constexpr (IS_TRACED) {
          HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 3");
    ...
    {
      if constexpr (IS_TRACED) {
          HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 4");
      ...
      if constexpr (IS_TRACED) {
          HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 5");
          // TODO: Provide appropriate set of flags.
          tracer->on_root_authority_finish(success, 0);
      }
  ...
  private:
    ...
    bool check_authority_impl( const authority& auth, uint32_t depth )
    {
      ...
      if constexpr (IS_TRACED) {
        HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 6");
      ...
      for( const auto& k : auth.key_auths )
      {
        if( signed_by( k.first ) )
        {
          ...
          if constexpr (IS_TRACED) {
            HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 7");
      ...
      if constexpr (IS_TRACED) {
        HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 8");
      ...
      for( const auto& a : auth.account_auths )
      {
        if( approved_by.find(a.first) == approved_by.end() )
        {
          ...
          {
            if constexpr (IS_TRACED) {
              HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 9");
          ...
          {
            if constexpr (IS_TRACED) {
              HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 10");
          ...
          if constexpr (IS_TRACED) {
            HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 11");
            try
            {
          ...
          if( success )
          {
            ...
            {
              if constexpr (IS_TRACED) {
                HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 12");
          ...
          if constexpr (IS_TRACED) {
            HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 13");
        ...
        {
          if constexpr (IS_TRACED) {
            HIVE_PROTOCOL_VALIDATION_ASSERT(tracer && "check_authority 14");
