...
namespace chainbase {
  ...
  class pool_allocator_t
    ...
    public:
      ...
      pool_allocator_t(const pool_allocator_t<T2, BLOCK_SIZE, TSegmentManager, USE_MANAGED_MAPPED_FILE>& other,
        ...
        : block_allocator(other.get_segment_manager())/*,
          blocks(typename decltype(blocks)::allocator_type(segment_manager)),
          free_list(typename decltype(free_list)::allocator_type(segment_manager))*/
        {
        }
      template <typename T2, bool _USE_MANAGED_MAPPED_FILE = USE_MANAGED_MAPPED_FILE>
      pool_allocator_t(const pool_allocator_t<T2, BLOCK_SIZE, TSegmentManager, USE_MANAGED_MAPPED_FILE>& other,
    ...
    private:
      struct block_t
        {
        T         memory[BLOCK_SIZE]; // allocated memory block
    ...
    private:
      ...
      T* get_object_memory()
        ...
        T* result = current_block->memory + current_block->first_free++;
