...
public:
    ...
    static bool lock_memory(const boost::interprocess::managed_mapped_file* segment) {
        ...
        ilog("Locking ${size} bytes of shared memory at ${addr} in RAM",
            ("size", size)("addr", addr));
        ...
        ilog("Successfully locked ${size} bytes of shared memory in RAM", ("size", aligned_size));
        return true;
#else
    ...
    static bool unlock_memory(const boost::interprocess::managed_mapped_file* segment) {
        ...
        ilog("Unlocking ${size} bytes of shared memory at ${addr}",
            ("size", size)("addr", addr));
        ...
        ilog("Successfully unlocked ${size} bytes of shared memory", ("size", aligned_size));
        return true;
#else
