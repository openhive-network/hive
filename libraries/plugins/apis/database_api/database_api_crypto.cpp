...
DEFINE_API_IMPL( database_api_impl, get_required_signatures )
{
  ...
  result.keys = args.trx.get_required_signatures(
    ...
    _db.get_chain_id(),
    args.available_keys,
    [&]( string account_name ){ return authority( _db.get< chain::account_authority_object, chain::by_account >( account_name ).get_active()  ); },
    [&]( string account_name ){ return authority( _db.get< chain::account_authority_object, chain::by_account >( account_name ).get_owner()   ); },
    [&]( string account_name ){ return authority( _db.get< chain::account_authority_object, chain::by_account >( account_name ).get_posting() ); },
...
DEFINE_API_IMPL( database_api_impl, verify_signatures )
{
  ...
  try
  {
    hive::protocol::verify_authority< verify_signatures_args >(
      ...
      { args },
      sig_keys,
      [this]( const string& name ) { return authority( _db.get< chain::account_authority_object, chain::by_account >( name ).get_owner() ); },
      [this]( const string& name ) { return authority( _db.get< chain::account_authority_object, chain::by_account >( name ).get_active() ); },
      [this]( const string& name ) { return authority( _db.get< chain::account_authority_object, chain::by_account >( name ).get_posting() ); },
