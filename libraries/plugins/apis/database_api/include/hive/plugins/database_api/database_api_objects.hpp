...
struct api_vesting_delegation_object
{
  api_vesting_delegation_object( const vesting_delegation_object& o, const database& db ):
    id( o.get_id() ),
    delegator( db.get_volatile_account( o.get_delegator() )->get_name() ),
    delegatee( db.get_volatile_account( o.get_delegatee() )->get_name() ),
...
struct api_convert_request_object
{
  ...
  api_convert_request_object( const convert_request_object& o, const database& db ):
    id( o.get_id() ),
    owner( db.get_volatile_account( o.get_owner() )->get_name() ),
...
struct api_comment_object
{
    ...
    depth( o.get_depth() )
  {
    ...
    if( cc )
    {
      ...
      {
        beneficiaries.emplace_back( db.get_volatile_account( route.account_id )->get_name(), route.weight );
      }
...
struct api_comment_vote_object
{
    ...
    num_changes( cv.get_number_of_changes() )
  {
    ...
    author = db.get_volatile_account( cc->get_author_id() )->get_name();
...
struct api_account_object
{
  ...
  {
    if( a.has_proxy() )
      proxy = db.get_volatile_account( a.get_proxy() )->get_name();
    if( a.has_recovery_account() )
      recovery_account = db.get_volatile_account( a.get_recovery_account() )->get_name();
    ...
    const auto& auth = db.get_volatile_account_authority( name );
    ...
    const auto maybe_meta = db.find_volatile_account_metadata( name );
    if( maybe_meta )
    {
