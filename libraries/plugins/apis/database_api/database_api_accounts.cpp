  ...
  switch( args.order )
  {
    case( by_name ):
    {
      iterate_results< chain::account_index, chain::by_name >(
        ...
        result.accounts,
        args.limit,
        [&]( const account_object& a, const database& db ){ return api_account_object( a, db, _metadata_api, args.delayed_votes_active ); },
...
DEFINE_API_IMPL( database_api_impl, find_accounts )
{
  ...
  for( auto& a : args.accounts )
  {
    ...
    if( acct != nullptr )
      result.accounts.emplace_back( *acct, _db, _metadata_api, args.delayed_votes_active );
  }
