...
DEFINE_API_IMPL( rc_api_impl, list_rc_direct_delegations )
{
  ...
  const auto* delegator = _db.find< account_object, hive::chain::by_name >( key[0].as< account_name_type >() );
  ...
  if (key[1].as< account_name_type >() != "") {
    const auto* delegatee = _db.find< account_object, hive::chain::by_name >( key[1].as< account_name_type >() );
  ...
  while( result.rc_direct_delegations.size() < args.limit && itr != end && itr->from == delegator->get_id())
  {
    ...
    const auto& to_account = _db.get<account_object, by_id>(rcdd.to);
