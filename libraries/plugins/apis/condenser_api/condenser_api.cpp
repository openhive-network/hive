...
namespace detail
{
  ...
  DEFINE_API_IMPL( condenser_api_impl, get_block )
  {
    ...
    if( b )
    {
      ...
      {
        result->transactions[i].transaction_id = b->transactions[i].id( _db.get_pack_flags() );
  ...
  DEFINE_API_IMPL( condenser_api_impl, get_transaction )
  {
    ...
    return hive::protocol::serializer_wrapper<annotated_signed_transaction>{ _account_history_api->get_transaction( { args[0].as< transaction_id_type >() } ), transaction_serialization_type::legacy };
  }
  ...
  DEFINE_API_IMPL( condenser_api_impl, broadcast_transaction_synchronous )
  {
    ...
    auto txid = trx.id( _db.get_pack_flags() );
