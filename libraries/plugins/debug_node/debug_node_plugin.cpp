...
const protocol::transaction_id_type& debug_node_plugin::make_artificial_transaction_for_debug_update()
{
  ...
  FC_ASSERT( my->_current_debug_update_txs.size() < HIVE_CUSTOM_OP_BLOCK_LIMIT, "Too many internal transactions." );
  ...
  fc::optional<protocol::transaction_id_type> transaction_id )
{
  ...
  debug_update( [ this, vest_modifier, hive_modifier ]( chain::database& db )
  {
    ...
    const auto& miner_assets = db.get< hive::chain::assets_object >( hive::chain::assets_object::id_type( miner_account.get_id().get_value() ) );
