...
class chain_plugin_impl
{
  public:
    ...
    void add_to_any_queue( t_write_queue& any_queue, const t_write_context_ptr& ctx )
    {
    ...
    void add_to_priority_write_queue( const t_write_context_ptr& ctx )
    {
      add_to_any_queue( priority_write_queue, ctx );
    }

    void add_to_write_queue( const t_write_context_ptr& ctx )
    {
    ...
    chain::database::signal_connection_ptr      end_of_sync_conn;
  ...
  write_processor_thread = std::make_shared<std::thread>([&]()
  {
    try
    {
      BOOST_SCOPE_EXIT(this_)
      {
        ...
        {
          std::lock_guard<std::mutex> _guard(this_->finish.mtx);
      ...
      while (true)
      {
        ...
        t_write_context_ptr cxt;
        {
          ...
          {
            std::lock_guard<std::mutex> _guard(finish.mtx);
            finish.status = true;
...

            {
              ...
              if (!running || (priority_write_queue.empty() && write_queue.empty()))
              {
                ...
                //if (write_queue_processed_duration > fc::milliseconds(500))
                  fc_dlog(fc::logger::get("chainlock"), "Emptied write_queue of ${write_queue_items_processed} items after ${write_queue_processed_duration}µs (${per_item}µs/item)",
                          (write_queue_items_processed)("write_queue_processed_duration", write_queue_processed_duration.count())
                          ("per_item", write_queue_processed_duration.count() / write_queue_items_processed));
                break;
              }
              if( not priority_write_queue.empty() )
              {
...
void chain_plugin_impl::open()
{
  ...
  try
  {
    ...
    db.with_write_lock([&]()
    {
      block_storage->open_and_init( bl_open_args, true/*read_only*/, true/*write_fallback*/, &db );
    });
        ...
        hive::chain::detail::without_pending_transactions( db, existing_block_flow_control( new_head_block ),
          [&]() {
            try
            {
              ...
              theApp.notify_fork(new_head_block_num, new_head_block_id.str());
