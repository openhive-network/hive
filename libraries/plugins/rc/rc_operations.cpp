...
{
  return dest.size() == HIVE_ASSET_SYMBOL_NAI_STRING_LENGTH - 1 && dest.c_str()[0] == '@' && dest.c_str()[1] == '@';
}
...
void set_slot_delegator_operation::validate() const
{
  ...
  FC_ASSERT( to_slot >= 0 );
  FC_ASSERT( to_slot < HIVE_RC_MAX_SLOTS );
}
...
void delegate_to_pool_evaluator::do_apply( const delegate_to_pool_operation& op )
{
  ...
  hive::chain::util::manabar_params rc_pool_manabar_params;
  rc_pool_manabar_params.regen_time = HIVE_RC_REGEN_TIME;
  if( !to_pool )
  {
...
void delegate_drc_from_pool_evaluator::do_apply( const delegate_drc_from_pool_operation& op )
{
  ...
  {
    hive::chain::util::manabar_params edge_manabar_params;
    edge_manabar_params.regen_time = HIVE_RC_REGEN_TIME;
  ...
  switch( op.to_slot )
  {
    case HIVE_RC_CREATOR_SLOT_NUM:
    {
      FC_ASSERT( (to_rca.creator == op.signer || op.signer == op.to_account ),
                 ...
                 ("c", to_rca.creator)("a", op.to_account)("n", HIVE_RC_CREATOR_SLOT_NUM) );
      break;
    }
    ...
    default:
    {
      FC_ASSERT( op.to_slot >= HIVE_RC_USER_SLOT_NUM && op.to_slot < HIVE_RC_MAX_SLOTS,
                 "User controlled slots must be between ${l} and ${u}",
                 ("l", HIVE_RC_USER_SLOT_NUM)("u", HIVE_RC_MAX_SLOTS) );
