 ...
 * slots for receiving delegation (`HIVE_RC_MAX_OUTDEL_SLOTS`).
 */
struct delegate_to_pool_operation : protocol::base_operation
{
 ...
 */
struct delegate_drc_from_pool_operation : protocol::base_operation
{
 ...
 * the existing delegator, the account itself, or (in the case of slot 1, HIVE_RC_RECOVERY_SLOT_NUM)
 * the slot may also be set by the account's recovery partner.
 */
struct set_slot_delegator_operation : protocol::base_operation
{
  account_name_type      from_pool;
  account_name_type      to_account;
  int8_t                 to_slot = 0;
...
HIVE_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, delegate_to_pool );
HIVE_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, delegate_drc_from_pool );
HIVE_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, set_slot_delegator );
...
FC_REFLECT( hive::plugins::rc::set_slot_delegator_operation,
  (from_pool)
  (to_account)
  (to_slot)
  (signer)
