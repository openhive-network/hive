...
void colony_plugin_impl::start( uint32_t block_num )
{
  ...
  for( const auto& account : accounts )
  {
    ...
    auto get_active = [&]( const std::string& name ) { return authority( _db.get< account_authority_object, by_account >( name ).get_active() ); };
    auto get_owner = [&]( const std::string& name ) { return authority( _db.get< account_authority_object, by_account >( name ).get_owner() ); };
    auto get_posting = [&]( const std::string& name ) { return authority( _db.get< account_authority_object, by_account >( name ).get_posting() ); };
