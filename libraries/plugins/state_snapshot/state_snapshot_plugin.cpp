...
std::tuple<snapshot_manifest, plugin_external_data_index, std::string, std::string, uint32_t> state_snapshot_plugin::impl::load_snapshot_manifest(const bfs::path& actualStoragePath)
{
...

  {
    ...
    std::unique_ptr<::rocksdb::Iterator> stateDefinitionsDataIterator(manifestDb->NewIterator(rOptions, cfHandles[5]));
    stateDefinitionsDataIterator->SeekToFirst();
    FC_ASSERT(stateDefinitionsDataIterator->Valid(), "No entry for BLOCKCHAIN_CONFIGURATION. Probably used old snapshot format (must be regenerated).");
