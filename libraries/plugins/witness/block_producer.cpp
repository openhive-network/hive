...
chain::signed_block block_producer::_generate_block(fc::time_point_sec when, const chain::account_name_type& witness_owner, const fc::ecc::private_key& block_signing_private_key)
{
  ...
  {
    FC_ASSERT( fc::raw::pack_size(pending_block, _db.get_pack_flags()) <= HIVE_MAX_BLOCK_SIZE );
  }
      ...
      chain::signed_block& pending_block)
{
  ...
  size_t total_block_size = fc::raw::pack_size( pending_block, _db.get_pack_flags() ) + 4;
  ...
  for( const chain::signed_transaction& tx : _db._pending_tx )
  {
    ...
    uint64_t new_total_size = total_block_size + fc::raw::pack_size( tx, _db.get_pack_flags() );
  ...
  {
    uint64_t new_total_size = total_block_size + fc::raw::pack_size( pending_required_itr->action, _db.get_pack_flags() );
...
FC_TODO( "Remove ifdef when required actions are added" )
  ...
  {
    uint64_t new_total_size = total_block_size + fc::raw::pack_size( pending_optional_itr->action, _db.get_pack_flags() );
