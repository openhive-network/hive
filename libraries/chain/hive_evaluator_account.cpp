  ...
  const account_object* recovery_account, const VEST_asset& initial_delegation )
{
  ...
  db.create< assets_object >( new_account.get_id(), new_account.get_name(), initial_delegation, _creation_time, mana_100_percent, rc_adjustment_from_fee );
...
void account_create_with_delegation_evaluator::do_apply( const account_create_with_delegation_operation& o )
{
  ...
  const auto& creator = _db.get_account( o.creator );
  const auto& creator_assets = _db.get< assets_object >( assets_object::id_type( creator.get_id().get_value() ) );
  const auto& props = _db.get_dynamic_global_properties();
  const witness_schedule_object& wso = _db.get_witness_schedule_object();
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
  ...
  const auto& recovery_obj = _db.get< recovery_object >( recovery_object::id_type( account_to_recover.get_id().get_value() ) );
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
  ...
  const auto& recovery_obj = _db.get< recovery_object >( recovery_object::id_type( account.get_id().get_value() ) );
