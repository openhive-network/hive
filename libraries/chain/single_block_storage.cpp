...
void single_block_storage::open_and_init( const block_log_open_args& bl_open_args, bool read_only,
  last_irreversible_block_access_i* lib_access )
{
  _open_args = bl_open_args;
  _lib_access = lib_access;
}
...
{
  _lib_access->set_last_irreversible_block_data( full_block );
}
...
{
  return _lib_access->get_last_irreversible_block_data();
}
