...
namespace hive { namespace chain {
  ...
  void block_log::open(const fc::path& file, bool read_only /* = false */, bool auto_open_artifacts /*= true*/ )
  {
      ...
      if (block_log_size)
      {
        ...
        head_block_num = my->head.load()->block_num();
      }
  ...
  {
    auto uncompressed_block_data = decompress_raw_block(raw_block_data, raw_block_size, attributes);

    signed_block_header block_header;
    fc::raw::unpack_from_char_array(std::get<0>(uncompressed_block_data).get(), std::get<1>(uncompressed_block_data), block_header);

    const block_id_type& block_id = block_header.id();

    return append_raw(block_num, raw_block_data, raw_block_size, attributes, block_id);
  }
  ...
  uint64_t block_log::append( const signed_block& b )
  {
    try
    {
      ...
      const block_id_type& block_id = b.id();
      ...
      if (my->compression_enabled)
      {
        try
        {
          ...
          attributes.dictionary_number = get_best_available_zstd_compression_dictionary_number_for_block(block_num);
          ...
          block_start_pos = append_raw(block_num, std::get<0>(zstd_compressed_block).get(), std::get<1>(zstd_compressed_block), attributes, block_id);
        }
        catch (const fc::exception&)
        {
          // compression failed for some unknown reason, store the block uncompressed
          block_start_pos = append_raw(block_num, serialized_block.data(), serialized_block.size(), {block_flags::uncompressed}, block_id);
        }
      }
  ...
  signed_block_header block_log::read_block_header_by_offset(uint64_t file_position, uint32_t block_serialized_data_size,
    const block_attributes_t& attrs) const
  {
    std::unique_ptr<char[]> serialized_data(new char[block_serialized_data_size]);
    auto total_read = detail::block_log_impl::pread_with_retry(my->block_log_fd, serialized_data.get(), block_serialized_data_size, file_position);
    FC_ASSERT(total_read == block_serialized_data_size);

    auto uncompressed_block_data = decompress_raw_block(serialized_data.get(), block_serialized_data_size, attrs);
    ...
    try
    {
      ...
      if (first_block_num <= last_block_num_from_disk)
      {
        ...
        for(size_t i = 0; i < block_artifacts.size(); ++i)
        {
          ...
          auto decompressed_raw_block = decompress_raw_block(block_data.get() + offset_in_memory, ba.block_serialized_data_size, ba.attributes);
