  ...
  try
  {
    ...
    else
    {
      if( has_hardfork( HIVE_HARDFORK_0_20 ) )
      {
        ...
        {
          util::update_manabar( cprops, to_account, a, a, new_vesting.amount.value );
        });
...
void database::clear_account( const account_object& account )
{
  ...
  if( assets.get_vesting().amount > 0 )
  {
    ...
    while( delegation_itr != delegation_idx.end() && delegation_itr->get_delegator() == account.get_id() )
    {
      ...
      {
        util::update_manabar( cprops, delegatee, a, a );
    ...
    // Update manabars on assets_object
    modify( assets, [&]( assets_object& m )
    {
      util::update_manabar( cprops, account, m, m );
...
void database::clear_expired_delegations()
{
  ...
  while( itr != delegations_by_exp.end() && itr->get_expiration_time() < now )
  {
    ...
    if( has_hardfork( HIVE_HARDFORK_0_20 ) )
    {
      ...
      {
        util::update_manabar( gpo, delegator, a, a, itr->get_vesting().amount.value );
      });
