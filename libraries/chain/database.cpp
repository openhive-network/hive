...
const time_object& database::get_time_account( const account_id_type& id )const
{
  return *( get_accounts_handler().get_time_account( id, true /*is_required*/ ) );
}

const time_object* database::find_time_account( const account_id_type& id )const
{
  return get_accounts_handler().get_time_account( id, false /*is_required*/ );
}
  ...
  try
  {
    ...
    else
    {
      if( has_hardfork( HIVE_HARDFORK_0_20 ) )
      {
        ...
        const auto& _time_obj = get_time_account( to_account.get_id() );
        ...
        {
          util::update_manabar( cprops, to_account, _assets_obj, _time_obj, mrc, new_vesting.get_amount() );
        });
...
void database::clear_null_account_balance()
{
  ...
  if( null_assets.get_vesting().amount > 0 )
  {
    ...
    const auto& null_time = get_time_account( null_account.get_id() );
...
void database::clear_account( const account_object& account )
{
  ...
  const auto& time = get_time_account( account.get_id() );
  ...
  if( assets.get_vesting().amount > 0 )
  {
    ...
    while( delegation_itr != delegation_idx.end() && delegation_itr->get_delegator() == account.get_id() )
    {
      ...
      const auto& delegatee_time = get_time_account( delegatee.get_id() );
...
void database::clear_expired_delegations()
{
  ...
  while( itr != delegations_by_exp.end() && itr->get_expiration_time() < now )
  {
    ...
    const auto& delegator_time = get_time_account( delegator.get_id() );
...
void database::adjust_balance( const account_object& a, const HIVE_asset& delta )
{
  ...
  const auto& acnt_time = get_time_account( a.get_id() );
  ...
  if( delta.symbol.asset_num == HIVE_ASSET_NUM_HIVE )
  {
    modify( acnt_assets, [&]( assets_object& acnt )
    {
      ...
      acnt.set_balance( acnt.get_balance() + HIVE_asset( delta ) );
      if(trace_balance_change)
...
void database::adjust_balance( const account_object& a, const HBD_asset& delta )
{
  if( delta.amount < 0 )
  {
    const auto& check_assets = get< assets_object, by_account_id >( a.get_id() );
    FC_ASSERT( check_assets.get_hbd_balance() >= -delta,
      "Account ${acc} does not have sufficient funds for balance adjustment. Required: ${r}, Available: ${a}",
        ("acc", a.get_name())("r", delta)("a", check_assets.get_hbd_balance()) );
  }
...
void database::adjust_reward_balance( const account_object& a, const HIVE_asset& value_delta )
{
  ...
  modify( _assets, [&]( assets_object& acnt )
  {
