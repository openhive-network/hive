  ...
  blockchain_worker_thread_pool& thread_pool, bool read_only, bool allow_artifacts_regeneration /*= false*/ )
{
  ...
  if( the_path.filename().string() == block_log_file_name_info::_legacy_file_name )
  {
    ...
    writer->open_and_init( the_path, read_only, allow_artifacts_regeneration );
    return writer;
  }
  ...
  if( part_number > 0 )
  {
    if( part_number == 1 )
    {
      ...
      writer->open_and_init( the_path, read_only, allow_artifacts_regeneration, 1/*start_from_part*/ );
      return writer;
    }
    else
    {
      ...
      writer->internal_open_and_init( single_log, the_path, read_only, allow_artifacts_regeneration );
  ...
  bool allow_artifacts_regeneration, database* db )
{
  ...
  common_open_and_init( read_only, allow_artifacts_regeneration, true /*allow_splitting_monolithic_log*/,
    db ? db->get_last_irreversible_block_data() : full_block_ptr_t() /*state_head_block*/ );
}

void block_log_wrapper::open_and_init( const fc::path& path, bool read_only, bool allow_artifacts_regeneration,
  uint32_t start_from_part /*= 1*/ )
{
  _open_args.data_dir = path.parent_path();
  common_open_and_init( read_only, allow_artifacts_regeneration, false /*allow_splitting_monolithic_log*/,
...
void block_log_wrapper::internal_open_and_init( block_log_ptr_t the_log, const fc::path& path,
                                                bool read_only, bool allow_artifacts_regeneration )
{
  the_log->open_and_init( path,
                          read_only,
                          allow_artifacts_regeneration,
...
void block_log_wrapper::common_open_and_init( bool read_only, bool allow_artifacts_regeneration,
  ...
  full_block_ptr_t state_head_block, uint32_t start_from_part /*= 1*/ )
{
  ...
  if( _block_log_split == LEGACY_SINGLE_FILE_BLOCK_LOG )
  {
    ...
    internal_open_and_init( single_part_log, log_file_path, read_only, allow_artifacts_regeneration );
  ...
  if( part_file_names.empty() )
  {
    ...
    else
    {
      ...
      internal_open_and_init( first_part_log, part_file_path, read_only, allow_artifacts_regeneration );
...
void block_log_wrapper::internal_append( uint32_t first_block_num, size_t block_count, append_t do_appending)
{
  ...
  if( is_last_number_of_the_file( first_block_num -1 ) && ( block_part_number > _logs.size() ) )
  {
    ...
    internal_open_and_init( new_part_log, new_path, false /*read_only*/, true /*allow_artifacts_regeneration*/ );
