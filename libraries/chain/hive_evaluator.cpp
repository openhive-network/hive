...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
  ...
  if( !o.is_clearing_proxy() ) {
    ...
    delta[0] = account.get_direct_governance_vote_power( _db.get< assets_object >( assets_object::id_type( account.get_id().get_value() ) ), _db.get< delayed_votes_object >( delayed_votes_object::id_type( account.get_id().get_value() ) ) );
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
  ...
  if( itr == by_account_witness_idx.end() )
  {
    ...
    if( _db.has_hardfork( HIVE_HARDFORK_0_3 ) )
      _db.adjust_witness_vote( witness, voter.get_governance_vote_power( _db.get< assets_object >( assets_object::id_type( voter.get_id().get_value() ) ), _db.get< delayed_votes_object >( delayed_votes_object::id_type( voter.get_id().get_value() ) ) ) );
    else if( _db.has_hardfork( HIVE_HARDFORK_0_2 ) )
      _db.adjust_proxied_witness_votes( voter, voter.get_governance_vote_power( _db.get< assets_object >( assets_object::id_type( voter.get_id().get_value() ) ), _db.get< delayed_votes_object >( delayed_votes_object::id_type( voter.get_id().get_value() ) ) ) );
    else
    {
      _db.modify( witness, [&]( witness_object& w )
      {
        w.votes += voter.get_governance_vote_power( _db.get< assets_object >( assets_object::id_type( voter.get_id().get_value() ) ), _db.get< delayed_votes_object >( delayed_votes_object::id_type( voter.get_id().get_value() ) ) );
      } );
    }
