...
void pre_hf20_vote_evaluator( const vote_operation& o, database& _db )
{
  ...
  int64_t abs_rshares = fc::uint128_to_int64( ( uint128_t( voter_details.get_effective_vesting_shares( false ).value ) * used_power ) / HIVE_100_PERCENT );
...
void hf20_vote_evaluator( const vote_operation& o, database& _db )
{
  ...
  _db.modify( voter_details, [&]( account_details_object& voter_details )
  {
    ...
    {
      used_mana = ( uint128_t( voter_details.get_effective_vesting_shares().value ) * abs_weight * 60 * 60 * 24 ) / HIVE_100_PERCENT;
    }
