...
uint64_t dhf_processor::calculate_votes( uint32_t pid )
{
  ...
  while( found != pvidx.end() && found->proposal_id == pid )
  {
    ...
    if( !_voter.has_proxy() )
    {
      auto sum = _voter.get_governance_vote_power( db.get< assets_object >( assets_object::id_type( _voter.get_id().get_value() ) ), db.get< delayed_votes_object >( delayed_votes_object::id_type( _voter.get_id().get_value() ) ) );
      ret += sum.value;
    }
