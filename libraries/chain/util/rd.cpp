...
{
  FC_ASSERT( user_params.budget_per_time_unit >= HIVE_RD_MIN_BUDGET );
  FC_ASSERT( user_params.budget_per_time_unit <= HIVE_RD_MAX_BUDGET );
  FC_ASSERT( user_params.decay_per_time_unit >= HIVE_RD_MIN_DECAY );
  FC_ASSERT( user_params.decay_per_time_unit <= HIVE_RD_MAX_DECAY );
}
  ...
  rd_dynamics_params& dparams_out )
{
  ...
  //   HIVE_RD_DECAY_DENOM_SHIFT.  The interface is designed to support other values of this
  ...
  //   code so that it sets decay_per_time_unit_denom_shift = HIVE_RD_DECAY_DENOM_SHIFT,
