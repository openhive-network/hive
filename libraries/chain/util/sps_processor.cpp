...
{
  auto head_time = note.block.block_header.timestamp;
...
{
  if( !is_maintenance_period( note.block.block_header.timestamp ) )
    return;
...
{
  if( !is_daily_maintenance_period( note.block.block_header.timestamp ))
    return;
  ...
  db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& _dgpo )
  {
    //shifting from current time instead of proper maintenance time causes drift
    //when maintenance block was missed, but the fix is problematic - see MR!168 for details
    _dgpo.next_daily_maintenance_time = note.block.block_header.timestamp + fc::seconds( HIVE_DAILY_PROPOSAL_MAINTENANCE_PERIOD );
  } );
