...
class rocksdb_account_archive : public accounts_handler
{
  private:
    ...
    // Use a high limit to prevent archiving during replay.
    // With split architecture each account creates 3 objects (account + metadata + authority),
    // so this must be above 3 * NUMBER_OF_ACCOUNTS at the replay target block.
    // The limit is removed (set to 0) when sync completes via remove_objects_limit().
    size_t objects_limit = 10'000'000;
