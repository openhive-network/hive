...
class rocksdb_comment_archive : public comments_handler
{
  private:
    ...
    std::shared_ptr<comment_object> get_comment_impl( const comment_object::author_and_permlink_hash_type& hash, const char* extended_hash, size_t extended_hash_size ) const;
