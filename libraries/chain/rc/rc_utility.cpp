...
void delegate_rc_evaluator::do_apply( const delegate_rc_operation& op )
{
  ...
  {
    const auto& to_account = _db.get_account( to );
  ...
  // Get the minimum between the current RC and the maximum delegable RC, so that eve can't f.e. re-delegate delegated RC
  int64_t from_delegable_rc = std::min( from_account.get_maximum_rc( from_mrc, from_assets, true ).value, from_mrc.get_rc_manabar().current_mana );
  ...
  _db.modify< manabars_rc_object >( from_mrc, [&]( manabars_rc_object& mrc )
  {
    ...
    mrc.set_last_max_rc( from_account.get_maximum_rc( mrc, from_assets ) );
  } );
...
void resource_credits::regenerate_rc_mana( const account_object& account, const fc::time_point_sec now ) const
{
  ...
  regenerate_rc_mana( account, mrc, assets, now );
}
  ...
  const fc::time_point_sec now ) const
{
  ...
  mbparams.max_mana = account.get_maximum_rc( mrc, assets ).value;
  mbparams.regen_time = HIVE_RC_REGEN_TIME;

  try
  {
    // Check if RC state was never properly initialized (last_max_rc == 0 but current computed max_mana != 0).
    ...
    bool rc_uninitialized = ( mrc.get_last_max_rc().value == 0 && mbparams.max_mana != 0 );
    ...
    if( rc_uninitialized )
    {
      ...
      {
        mrc_obj.get_rc_manabar().last_update_time = now.sec_since_epoch();
        mrc_obj.get_rc_manabar().current_mana = mbparams.max_mana;
        mrc_obj.set_last_max_rc( mbparams.max_mana );
      } );
      return;
    }

    if( mbparams.max_mana != mrc.get_last_max_rc() )
    {
      ...
      HIVE_ASSERT( false, plugin_exception,
        ...
        ( "a", account.get_name() )( "old", mrc.get_last_max_rc() )( "new", mbparams.max_mana )( "b", db.head_block_num() ) );
#else
    ...
    db.modify( mrc, [&]( manabars_rc_object& mrc_obj )
    {
      mrc_obj.get_rc_manabar().regenerate_mana< true >( mbparams, now );
    } );
  ...
  const fc::time_point_sec now, int64_t delta, bool regenerate_mana ) const
{
  ...
  // This can happen in alternate chain configurations where hardforks are applied out of order.
  bool rc_uninitialized = ( mrc.get_last_max_rc().value == 0 );
  if( rc_uninitialized )
  {
    ...
    if( max_rc != 0 )
    {
      ...
      {
        mrc_obj.get_rc_manabar().last_update_time = now.sec_since_epoch();
        mrc_obj.get_rc_manabar().current_mana = max_rc;
        mrc_obj.set_last_max_rc( max_rc );
      } );
    }
  }

  db.modify< manabars_rc_object >( mrc, [&]( manabars_rc_object& mrc_obj )
  {
    auto max_rc = account.get_maximum_rc( mrc, assets ).value;
    util::manabar_params manabar_params( max_rc, HIVE_RC_REGEN_TIME );
    if( regenerate_mana )
    {
      mrc_obj.get_rc_manabar().regenerate_mana< true >( manabar_params, now );
    }
    else
    {
      bool regenerate_condition_rc = (mrc_obj.get_rc_manabar().last_update_time != now.sec_since_epoch());
    ...
    mrc_obj.get_rc_manabar().current_mana = std::max( mrc_obj.get_rc_manabar().current_mana + delta, int64_t( 0 ) );
    mrc_obj.set_last_max_rc( max_rc + delta );
    mrc_obj.set_received_rc( mrc_obj.get_received_rc() + delta );
  } );
}
  ...
  const fc::time_point_sec now, bool _fill_new_mana, bool _check_for_rc_delegation_overflow ) const
{
  const auto& mrc = db.get_manabars_rc_account( account.get_id() );
  const auto& assets = db.get_asset_account( account.get_id() );
  update_account_after_vest_change( account, mrc, assets, now, _fill_new_mana, _check_for_rc_delegation_overflow );
}
  ...
  const fc::time_point_sec now, bool _fill_new_mana, bool _check_for_rc_delegation_overflow ) const
{
  ...
  if( _check_for_rc_delegation_overflow && mrc.get_delegated_rc() > 0 )
  {
    ...
    int64_t delegation_overflow = -account.get_maximum_rc( mrc, assets, true ).value;
    ...
    if( delegation_overflow > 0 )
    {
      ...
      db.modify( mrc, [&]( manabars_rc_object& mrc_obj )
      {
        mrc_obj.set_delegated_rc( mrc_obj.get_delegated_rc() - initial_overflow );
      } );
    }
  }

  int64_t new_last_max_rc = account.get_maximum_rc( mrc, assets ).value;
  int64_t drc = new_last_max_rc - mrc.get_last_max_rc().value;
  drc = _fill_new_mana ? drc : 0;

  if( new_last_max_rc != mrc.get_last_max_rc() )
  {
    db.modify( mrc, [&]( manabars_rc_object& mrc_obj )
    {
      ...
      mrc_obj.set_last_max_rc( new_last_max_rc );
      ...
      mrc_obj.get_rc_manabar().current_mana += std::max( drc, int64_t( 0 ) );
      //...however we have to reduce it when its current level would exceed new maximum (issue #191)
      if( mrc_obj.get_rc_manabar().current_mana > mrc_obj.get_last_max_rc() )
        mrc_obj.get_rc_manabar().current_mana = mrc_obj.get_last_max_rc().value;
    } );
  }
}
...
bool resource_credits::use_account_rcs( int64_t rc )
{
  ...
  util::manabar_params mbparams;
  auto max_mana = account.get_maximum_rc( mrc, assets ).value;
  mbparams.max_mana = max_mana;
  tx_info.max = max_mana;
  tx_info.rc = mrc.get_rc_manabar().current_mana; // initialize before regen in case of exception
  mbparams.regen_time = HIVE_RC_REGEN_TIME;
  ...
  try
  {
    db.modify( mrc, [&]( manabars_rc_object& mrc_obj )
    {
      mrc_obj.get_rc_manabar().regenerate_mana< true >( mbparams, dgpo.time.sec_since_epoch() );
      tx_info.rc = mrc_obj.get_rc_manabar().current_mana; // update after regeneration
      int64_t surcharge = 0;
      ...
      bool has_mana = mrc_obj.get_rc_manabar().has_mana( rc + surcharge );
...
#endif
      {
        ...
        {
          if( surcharge && mrc_obj.get_rc_manabar().has_mana( rc ) )
          {
      ...
      mrc_obj.get_rc_manabar().use_mana( rc + surcharge );
      tx_info.rc = mrc_obj.get_rc_manabar().current_mana;
    } );
  ...
  const rc_pool_object& rc_pool ) const
{
  ...
  if( auto_report_type != report_type::NONE && new_stats_obj.get_starting_block() )
  {
    ...
    case report_output::NOTIFY:
      db.get_app().notify_information( "RC stats", "rc_stats", report );
