...
void delegate_rc_evaluator::do_apply( const delegate_rc_operation& op )
{
  ...
  _db.modify( from_assets, [&]( assets_object& a )
  {
    ...
    a.set_delegated_rc( a.get_delegated_rc() + delta_total );
    a.set_last_max_rc( from_account.get_maximum_rc( a ) );
  } );
  ...
  const fc::time_point_sec now ) const
{
  ...
  try
  {
    ...
    {
      a.get_rc_manabar().regenerate_mana< true >( mbparams, now );
    } );
  ...
  const fc::time_point_sec now, int64_t delta, bool regenerate_mana ) const
{
  ...
  db.modify( assets, [&]( assets_object& a )
  {
    ...
    if( regenerate_mana )
    {
      a.get_rc_manabar().regenerate_mana< true >( manabar_params, now );
    }
    else
    {
      bool regenerate_condition_rc = (a.get_rc_manabar().last_update_time != now.sec_since_epoch());
    ...
    a.get_rc_manabar().current_mana = std::max( a.get_rc_manabar().current_mana + delta, int64_t( 0 ) );
    a.set_last_max_rc( max_rc + delta );
    a.set_received_rc( a.get_received_rc() + delta );
  } );
}
  ...
  const fc::time_point_sec now, bool _fill_new_mana, bool _check_for_rc_delegation_overflow ) const
{
  ...
  if( _check_for_rc_delegation_overflow && assets.get_delegated_rc() > 0 )
  {
    ...
    if( delegation_overflow > 0 )
    {
      ...
      db.modify( assets, [&]( assets_object& a )
      {
        a.set_delegated_rc( a.get_delegated_rc() - initial_overflow );
      } );
    }
  }
  ...
  if( new_last_max_rc != assets.get_last_max_rc() )
  {
    db.modify( assets, [&]( assets_object& a )
    {
      ...
      a.set_last_max_rc( new_last_max_rc );
      ...
      a.get_rc_manabar().current_mana += std::max( drc, int64_t( 0 ) );
      //...however we have to reduce it when its current level would exceed new maximum (issue #191)
      if( a.get_rc_manabar().current_mana > a.get_last_max_rc() )
        a.get_rc_manabar().current_mana = a.get_last_max_rc().value;
    } );
  }
}
...
bool resource_credits::use_account_rcs( int64_t rc )
{
  ...
  try
  {
    db.modify( assets, [&]( assets_object& a )
    {
      a.get_rc_manabar().regenerate_mana< true >( mbparams, dgpo.time.sec_since_epoch() );
      tx_info.rc = a.get_rc_manabar().current_mana; // update after regeneration
      int64_t surcharge = 0;
      ...
      bool has_mana = a.get_rc_manabar().has_mana( rc + surcharge );
...
#endif
      {
        ...
        {
          if( surcharge && a.get_rc_manabar().has_mana( rc ) )
          {
      ...
      a.get_rc_manabar().use_mana( rc + surcharge );
      tx_info.rc = a.get_rc_manabar().current_mana;
    } );
