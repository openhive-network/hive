...
BOOST_AUTO_TEST_CASE(blowfish_ecb_test)
{
    for ( int i = 0; i < 34; i++ ) {
        ...
        BOOST_CHECK_EQUAL( static_cast< size_t >( 8 ), fc::from_hex( ecb_tests[i].key, (char*) key, sizeof(key) ) );
        BOOST_CHECK_EQUAL( static_cast< size_t >( 8 ), fc::from_hex( ecb_tests[i].plain, (char*) plain, sizeof(plain) ) );
        BOOST_CHECK_EQUAL( static_cast< size_t >( 8 ), fc::from_hex( ecb_tests[i].cipher, (char*) cipher, sizeof(cipher) ) );
...
BOOST_AUTO_TEST_CASE(blowfish_key_test)
{
    ...
    BOOST_CHECK_EQUAL( static_cast< size_t >( 24 ), fc::from_hex( key_test_key.c_str(), (char*) key, sizeof(key) ) );
    BOOST_CHECK_EQUAL( static_cast< size_t >( 8 ), fc::from_hex( key_test_plain.c_str(), (char*) plain, sizeof(plain) ) );

    for ( unsigned int i = 0; i < sizeof(key); i++ ) {
        BOOST_CHECK_EQUAL( static_cast< size_t >( 8 ), fc::from_hex( key_test_ciphers[i], (char*) cipher, sizeof(cipher) ) );
        fc::blowfish fish;
        fish.start( key, i + 1 );
        fish.encrypt( plain, out, 8, fc::blowfish::ECB );
        BOOST_CHECK( !memcmp( cipher, out, 8) );
...
BOOST_AUTO_TEST_CASE(blowfish_chain_test)
{
    ...
    BOOST_CHECK_EQUAL( static_cast< size_t >( 16 ), fc::from_hex( chain_test_key.c_str(), (char*) key, sizeof(key) ) );
    BOOST_CHECK_EQUAL( static_cast< size_t >( 8 ), fc::from_hex( chain_test_iv.c_str(), (char*) iv, sizeof(iv) ) );

    BOOST_CHECK_EQUAL( static_cast< size_t >( 32 ), fc::from_hex( chain_test_cbc.c_str(), (char*) cipher, sizeof(cipher) ) );
    fc::blowfish fish;
    ...
    BOOST_CHECK_EQUAL( static_cast< size_t >( 29 ), fc::from_hex( chain_test_cfb.c_str(), (char*) cipher, sizeof(cipher) ) );
    fish.reset_chain();
