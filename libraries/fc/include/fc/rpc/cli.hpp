...
namespace fc { namespace rpc {
   ...
   class cli : public api_connection
   {
      public:
         ...
         void stop();
         void wait();
         void format_result( const string& method, std::function<variant(const variant&)> formatter);
      ...
      private:
         ...
         std::string _prompt = ">>>";
         std::map<string, std::function<variant(const variant&)> > _result_formatters;
