...
INSERT INTO public.internal_data( first_block, last_block ) VALUES( 0, 0 )

DROP MATERIALIZED VIEW IF EXISTS account_operations_view;
CREATE MATERIALIZED VIEW account_operations_view AS
...
CREATE INDEX IF NOT EXISTS account_operations_view_ix1 ON account_operations_view (id);
CREATE INDEX IF NOT EXISTS account_operations_view_ix2 ON account_operations_view (account_name);
...
BEGIN
	UPDATE public.internal_data
	SET first_block = _FIRST_BLOCK, last_block = _LAST_BLOCK;

	REFRESH MATERIALIZED VIEW account_operations_view;

	---------add `accounts`---------
	INSERT INTO public.accounts( name )
	SELECT DISTINCT aov.account_name
	FROM account_operations_view aov
	...
	SELECT pa.id, aov.id
	FROM account_operations_view aov JOIN public.accounts pa ON aov.account_name = pa.name;
