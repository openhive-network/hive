...
test.describe('WASM beekeeper_api tests for Node.js', () => {
  ...
  test('Basic tests which execute all endpoints', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ provider, BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);

      api.create_temporary_wallet_with_password(api.implicitSessionToken, walletNames[0], "this_is_password", true );
      {
        const wallets = api.listWallets(api.implicitSessionToken);
        requireWalletsIn(wallets, 1, 0, true);
      }
      api.importKey(api.implicitSessionToken, walletNames[0], keys[3][0]);
      {
        const publicKeys = api.getPublicKeys(api.implicitSessionToken);
        requireKeysIn(publicKeys, 3);
      }
      api.close(api.implicitSessionToken, walletNames[0])
      {
        const wallets = api.listWallets(api.implicitSessionToken);
        requireWalletsIn(wallets, 0);
      }
  ...
  test('Trying to open beekeeper instance again so as to find out if data created in previous test is permamently saved', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
  ...
  test('Remove all keys from 3 wallets created in previous step. As a result all keys are removed in mentioned wallets', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
  ...
  test('Unlock 13 wallets. From every wallet remove every key', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
  ...
  test('Close implicitly created session. Create 4 new sessions. All sessions open the same wallet', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
  ...
  test('False tests for every API endpoint', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
  ...
  test('Try to sign transactions, but they fail. Unlock wallet. Import key. Sign transactions. Delete instance. Create instance again', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      ...
      api.unlock(api.implicitSessionToken!, walletNames[7]);
      api.importKey(api.implicitSessionToken, walletNames[7], keys[3][0]);
  ...
  test('Analyze error messages', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
  ...
  test('An incorrect initialization should block all API calls', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ provider }) => {
  ...
  test('Check `encrypt_data`, `decrypt data` endpoints', async ({ beekeeperWasmTest }) => {
    await beekeeperWasmTest(async ({ BeekeeperInstanceHelper }) => {
      const api = new BeekeeperInstanceHelper(WALLET_OPTIONS_NODE);
