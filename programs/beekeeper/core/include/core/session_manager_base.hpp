...
class session_manager_base
{
  ...
  protected:
    ...
    virtual std::shared_ptr<session_base> create_session( const std::string& notifications_endpoint, const std::string& token, std::shared_ptr<time_manager_base> time, const boost::filesystem::path& wallet_directory );
  ...
  public:
    ...
    std::string create_session( const std::string& salt, const std::string& notifications_endpoint, const boost::filesystem::path& wallet_directory );
