...
template<typename T>
void function_wrapper(T function, const std::string function_name)
{
...
void do_job(const fc::path& input_block_log_path, const fc::path& output_block_log_path, const unsigned compress_jobs, const bool input_read_only)
{
  ...
  auto fpq = std::bind(fill_pending_queue, (input_block_log_path / "block_log"), input_read_only, std::placeholders::_1);
  auto dcq = std::bind(drain_completed_queue, (output_block_log_path / "block_log"), std::placeholders::_1);
  auto cb = std::bind(compress_blocks, std::placeholders::_1);

  std::vector<std::thread> workers{2 + compress_jobs};
