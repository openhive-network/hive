...
void test_pop( uint32_t nr_signatures, uint32_t nr_signatures_2, uint32_t nr_threads )
{
  ...
  std::vector<G2Element> _signatures = obj_00.sign_content_2();
  ...
  G2Element _aggregated_signatures = obj_00.aggregate_signatures( _signatures );
  ...
  _start = std::chrono::high_resolution_clock::now();
  bool _result = obj_00.verify( obj_00.public_keys, _aggregated_signatures );
  _interval = std::chrono::duration_cast<std::chrono::microseconds>( std::chrono::high_resolution_clock::now() - _start ).count();
