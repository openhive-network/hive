...
docker build --target=build \
  ...
  --tag "${REGISTRY}${IMAGE_TAG_PREFIX:-}/build:${BUILD_IMAGE_TAG}" \
...
docker build --target=instance \
  ...
  --build-arg HIVE_SUBDIR="$HIVE_SUBDIR" \
  --build-arg IMAGE_TAG_PREFIX="${IMAGE_TAG_PREFIX:+$IMAGE_TAG_PREFIX-}" \
  --build-context "build=docker-image://${REGISTRY}${IMAGE_TAG_PREFIX:-}/build:${BUILD_IMAGE_TAG}" \
...
if [ -n "${EXPORT_PATH}" ];
then
  "$SCRIPTPATH/export-data-from-docker-image.sh" "${REGISTRY}${IMAGE_TAG_PREFIX:-}/build:${BUILD_IMAGE_TAG}" "${EXPORT_PATH}"
fi
