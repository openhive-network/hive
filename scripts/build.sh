...
if [[ -d "$HIVED_INSTALLATION_DIR" ]]; then
    ...
    if [[ -f "$abs_build_dir/libraries/chainbase/test/std_allocator_test" ]]; then
        mv "$abs_build_dir/libraries/chainbase/test/std_allocator_test" "$HIVED_INSTALLATION_DIR/"
    fi
