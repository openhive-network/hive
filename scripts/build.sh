...
if [[ -d "$HIVED_INSTALLATION_DIR" ]]; then
    ...
    if [[ -n "$(shopt -s nullglob; echo "$abs_build_dir/tests/unit/"*)" ]]; then
        ...
        # This includes the test discovery files so ctest can find the tests
...
include("${CMAKE_CURRENT_LIST_DIR}/chain_test[1]_include.cmake" OPTIONAL)
include("${CMAKE_CURRENT_LIST_DIR}/plugin_test[1]_include.cmake" OPTIONAL)
EOF
    fi
