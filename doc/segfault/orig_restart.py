...
def get_config():
	...
	args.add_argument("--bpath", type=str, dest='hived', default='/home/raidg/hive/default_build/programs/hived/hived')
	...
	args.add_argument("--tries", type=int, dest='tries', default=10)
	...
	args.add_argument("--block-log", type=str, dest='block_log', default='/var/block_log/default_block_log')
...
def prepare_env( args, datadir ):
	...
	block_log_index = args.block_log + ".index"
	block_log_dir = join(datadir, "blockchain")
	block_log_dest = join(block_log_dir, "block_log")
	block_log_index_dest = block_log_dest + ".index"
	config_dest = join(datadir, "config.ini")
	purge = args.purge

	rmtree('log_dump', True)
	mkdir('log_dump')
	...
	mkdir(datadir) # datadir
	print(f"created datadir at: {datadir}")
	mkdir(block_log_dir) # datadir/blockchain
	symlink( args.block_log, block_log_dest ) # datadir/blockchain/block_log
	if exists(block_log_index):
		symlink( block_log_index, block_log_index_dest ) # datadir/blockchain/block_log.index
	...
	cfg.generate(config_dest)

	return [ args.hived, "--replay-blockchain", "-d", datadir, "--force-open"]

def check( args : str, sleep_time : int ):
	from subprocess import signal, Popen
	from time import sleep

	print(f"runnig hived:\t{' '.join(args)}")

	with open(f"log_dump/stdout_{sleep_time}.log", 'w') as STDOUT:
		with open(f"log_dump/stderr_{sleep_time}.log", 'w') as STDERR:
			with Popen(args=args, bufsize=0, stdout=STDOUT, stderr=STDERR) as process:
	...
	try:
		...
		with TemporaryDirectory() as tmpdir:
			...
			with ThreadPoolExecutor( max_workers=2 ) as pool:
				futures = [ pool.submit( call, RUN ) for _ in range(3) ]

				try:
					args = prepare_env(cfg, tmpdir)
					for _ in range(count):
						retcode = check( args, next(generator) )
						if retcode != 0:
