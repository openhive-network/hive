...
class Account:
    ...
    def __init__(self, name, secret='secret', with_keys=True):
        ...
        if with_keys:
            output = self.__key_generator.generate_keys(name, secret=secret)[0]
            self.secret = secret
    ...
    def create_multiple(cls, number_of_accounts, name_base='account', *, secret='secret'):
        accounts = []
        for generated in cls.__key_generator.generate_keys(name_base, number_of_accounts=number_of_accounts, secret=secret):
