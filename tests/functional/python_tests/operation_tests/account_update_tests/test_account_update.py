...
@pytest.mark.testnet
@pytest.mark.parametrize("authority_type, comparison_type", (
        ("active", "__eq__"),
        ("posting", "__eq__"),
        ("owner", "__gt__"),
))
...
def test_update_all_account_parameters_using_owner_authority(alice):
    ...
    new_json_meta = '{"foo": "bar"}'
    new_owner, new_active, new_posting, new_memo = (alice.generate_new_key(), alice.generate_new_key(),
                                                    alice.generate_new_key(), alice.generate_new_key())
    rc_before_operation = alice.get_rc_current_mana()
    alice.update_all_account_details(owner=new_owner, active=new_active, posting=new_posting, memo=new_memo,
                                     json_meta=new_json_meta)
    rc_after_operation = alice.get_rc_current_mana()
    alice.assert_account_details_were_changed(new_owner=new_owner, new_active=new_active, new_posting=new_posting,
                                              new_memo=new_memo, new_json_meta=new_json_meta)
...
def test_update_all_account_parameters_except_owner_key_using_active_authority(alice):
    ...
    new_json_meta = '{"foo": "bar"}'
    current_active, current_posting, new_memo = (alice.get_current_key("active"), alice.get_current_key("posting"),
    ...
    alice.assert_account_details_were_changed(new_active=[current_active, new_weight],
                                              new_posting=[current_posting, new_weight], new_memo=new_memo,
