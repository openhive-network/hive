...
@pytest.mark.testnet
def test_cancel_transfer_from_savings_simplest_scenario(prepared_node, wallet, alice, currency, check_savings_balance,
                                                        check_balance):
    ...
    assert balance_before_withdrawal == balance_after_withdrawal + currency(50), \
        f"{currency.token} savings balance wasn't decreased after withdrawal"
    assert rc_amount_before_sending_op > rc_amount_after_sending_op, f"RC amount after hive withdrawal " \
                                                                     f"wasn't decreased."
    prepared_node.wait_for_irreversible_block()

    prepared_node.restart(
        time_offset=tt.Time.serialize(prepared_node.get_head_block_time() + tt.Time.days(2),
    ...
    assert savings_balance_before_withdrawal_cancellation + currency(50) == \
           savings_balance_after_withdrawal_cancellation, (f"{currency.token}S from cancelled withdrawal didn't come "
                                                           f"back to savings balance")

    prepared_node.restart(
        time_offset=tt.Time.serialize(prepared_node.get_head_block_time() + tt.Time.days(1),
                                      format_=tt.Time.TIME_OFFSET_FORMAT)
    )

    assert getattr(alice, check_balance)() - funds_after_transfer_to_savings == currency(0), \
...
@pytest.mark.testnet
def test_cancel_all_transfers_from_savings(prepared_node, wallet, alice, currency, check_savings_balance,
                                           check_balance):
    create_three_savings_withdrawals_from_fresh_account(prepared_node, currency, alice, check_savings_balance,
                                                        currency.token)
    funds_after_transfer_to_savings = getattr(alice, check_balance)()
    ...
    prepared_node.restart(
        time_offset=tt.Time.serialize(prepared_node.get_head_block_time() + tt.Time.days(3),
                                      format_=tt.Time.TIME_OFFSET_FORMAT)
        )
...
@pytest.mark.testnet
def test_cancel_all_transfers_from_savings_except_one(prepared_node, wallet, alice, currency, check_savings_balance,
                                                      check_balance):
    create_three_savings_withdrawals_from_fresh_account(prepared_node, currency, alice, check_savings_balance,
                                                        currency.token)
    funds_after_transfer_to_savings = getattr(alice, check_balance)()
    ...
    assert getattr(alice, check_balance)() - funds_after_transfer_to_savings == currency(10), \
        (f"At least one of cancelled withdrawals arrived to receiver account {currency.token} balance or the "
         f"withdrawal that wasn't cancelled didn't arrive.")

    assert check_if_fill_transfer_from_savings_vop_was_generated(
        prepared_node, f"transfer from savings with id: 1"
    ) is True, f"fill_transfer_from_savings from transfer with id 1 wasn't generated"
...
def create_three_savings_withdrawals_from_fresh_account(node, currency, account, check_savings_balance, token):
    ...
    for time_offset in (6, 12, 18):
        ...
        assert rc_amount_before_sending_op > rc_amount_after_sending_op, \
            f"RC amount after {token} withdrawal wasn't decreased."
        assert balance_before_withdrawal == balance_after_withdrawal + amount, \
            f"{token} savings balance wasn't decreased after withdrawal"

        amount += currency(5)
        withdrawal_id += 1
        node.restart(
            time_offset=tt.Time.serialize(node.get_head_block_time() + tt.Time.hours(time_offset),
                                          format_=tt.Time.TIME_OFFSET_FORMAT)
        )
