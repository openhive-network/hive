...
@pytest.mark.testnet
def test_transfer_from_savings_account(prepared_node, wallet, alice, bob, receiver_of_savings_withdrawal, currency,
                                       check_balance, check_savings_balance):
    ...
    alice.transfer_from_savings(0, receiver_of_savings_withdrawal, currency(5), "transfer from savings")
    ...
    assert rc_amount_before_sending_op > rc_amount_after_sending_op, "RC amount after withdrawing hive wasn't " \
                                                                     "decreased."
    assert savings_balance_before_withdrawal - currency(5) == savings_balance_after_withdrawal, \
...
@pytest.mark.testnet
def test_transfer_from_savings_during_few_days(prepared_node, wallet, alice, currency, check_savings_balance,
                                               check_balance):
    ...
    amount_to_transfer = currency(0)
    sum_of_completed_transfers = currency(0)
    ...
    for day_number in range(8):
        amount_to_transfer += currency(5)
        if day_number < 5:
            transfer_id = day_number
            balance_before_withdrawal = getattr(alice, check_savings_balance)()
            rc_amount_before_sending_op = alice.get_rc_current_mana()
            wallet.api.transfer_from_savings("alice", transfer_id, "alice", amount_to_transfer,
                                             f"transfer from savings with id:{transfer_id}")
            rc_amount_after_sending_op = alice.get_rc_current_mana()
        ...
        if day_number >= 3:
            ...
            sum_of_completed_transfers += amount_to_transfer - currency(15)

            assert (
                    getattr(alice, check_balance)() - funds_after_transfer_to_savings
                    == sum_of_completed_transfers
            ...
            ) is True, f"fill_transfer_from_savings from transfer with id:{completed_transfer_id} wasn't generated"
