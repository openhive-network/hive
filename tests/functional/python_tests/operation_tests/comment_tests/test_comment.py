...
@pytest.mark.parametrize("reply_type", ['reply_own_comment', 'reply_another_comment'], ids=['comment own post', 'comment someone else post'])
...
def test_update_comment_with_replies_votes_and_downvotes(wallet, prepared_node, reply_type):
  ...
  # Not in single transaction, because it is possible to vote every 3 seconds.
    ...
    assert reward_hbd_balances[2] < reward_hbd_balances[3], "HBD not reduce. Downvoting does not work correctly after comment update."
    assert reward_hbd_balances[0] < reward_hbd_balances[1], "HBD not reduce. Downvoting does not work correctly."
    assert reward_hbd_balances[0] == reward_hbd_balances[2], "Account update have an impact on voting. Votes are changed"
    assert reward_hbd_balances[1] == reward_hbd_balances[3], "Account update have an impact on downvoting. Downvotes are changed"
