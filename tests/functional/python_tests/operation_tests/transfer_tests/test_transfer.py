...
def test_transfer(alice, bob, prepared_node, wallet, amount, memo, check_balance):
    ...
    assert sender_balance_before_transfer == sender_balance_after_transfer + amount, \
        f"{amount.token} balance of sender wasn't decreased."
    assert receiver_balance_before_transfer == receiver_balance_after_transfer - \
           amount, f"{amount.token} balance of receiver wasn't increased."
...
def test_transfer_hives_to_hive_fund_account(prepared_node, wallet, alice, hive_fund):
    ...
    assert sender_balance_before_sending_hives == sender_balance_after_sending_hives + \
           tt.Asset.Test(5), "HIVE balance of sender wasn't decreased."
    assert receiver_balance_before_sending_hives < receiver_balance_after_sending_hives, "HIVE balance of receiver" \
                                                                                         " wasn't decreased."
    assert rc_amount_before_sending_op > rc_amount_after_sending_op, "RC amount after sending transfer wasn't " \
                                                                     "decreased."
