...
@pytest.mark.testnet
def test_not_matching_orders_with_fill_or_kill(prepared_node, alice, bob, use_hbd_in_matching_order, create_main_order,
                                               create_normal_order):
    """
...
def test_match_third_order_with_kill_or_fill(prepared_node, alice, bob, carol, use_hbd_in_matching_order,
                                             create_main_order, create_normal_order):
    """
...
def test_match_fifth_order(prepared_node, alice, bob, carol, daisy, elizabeth, use_hbd_in_matching_order,
    ...
    for account, amount, condition in zip((alice, bob, carol, daisy, elizabeth, elizabeth), (550, 400, 500, 480,
                                                                                             836.666, 300), check_hbd):
        account.assert_balance(amount=amount, check_hbd=condition, message="order_match")

    # check if rc was reduced after fixing issue: https://gitlab.syncad.com/hive/hive/-/issues/507
    assert get_number_of_fill_order_operations(prepared_node) == 3, \
        "fill_order_operation virtual operation wasn't generated"


@pytest.mark.parametrize(parametrize_variables, parametrize_values)
@pytest.mark.testnet
def test_not_matching_orders(prepared_node, alice, bob, use_hbd_in_matching_order, create_main_order,
                             create_normal_order):
    """
...
@pytest.mark.parametrize(parametrize_variables, parametrize_values)
@pytest.mark.testnet
def test_full_match_third_order(prepared_node, alice, bob, carol, use_hbd_in_matching_order, create_main_order,
                                create_normal_order):
    """
...
@pytest.mark.parametrize(parametrize_variables, parametrize_values)
@pytest.mark.testnet
def test_full_match_first_order(prepared_node, alice, bob, carol, use_hbd_in_matching_order, create_main_order,
                                create_normal_order):
    """
...
def test_partially_match_every_next_order(prepared_node, alice, bob, carol, daisy, elizabeth, use_hbd_in_matching_order,
    ...
    for account, amount, condition in zip((alice, daisy), (650, 630),
                                          (not use_hbd_in_matching_order, use_hbd_in_matching_order)):
        account.assert_balance(amount=amount, check_hbd=condition, message="order_match")
    ...
    for account, amount, condition in zip((alice, elizabeth), (750, 750),
                                          (not use_hbd_in_matching_order, use_hbd_in_matching_order)):
        account.assert_balance(amount=amount, check_hbd=condition, message="order_match")
...
def test_match_third_order_partially_and_wait_for_expiration(prepared_node, alice, bob, carol,
                                                             use_hbd_in_matching_order, create_main_order,
                                                             create_normal_order):
    """
    ...
    for account, amount, condition in zip((alice, carol), (610, 700),
                                          (use_hbd_in_matching_order, not use_hbd_in_matching_order)):
        account.assert_balance(amount=amount, check_hbd=condition, message="order_match")
...
def test_match_third_order_partially_and_wait_for_expiration_scenario_2(prepared_node, alice, bob, carol,
    ...
    for account, amount, condition in zip((alice, carol), (550, 550),
                                          (not use_hbd_in_matching_order, use_hbd_in_matching_order)):
        account.assert_balance(amount=amount, check_hbd=condition, message="order_match")
...
@pytest.mark.parametrize(parametrize_variables, parametrize_values)
@pytest.mark.testnet
def test_full_match_fourth_order(prepared_node, alice, bob, carol, daisy, use_hbd_in_matching_order, create_main_order,
                                 create_normal_order):
    """
