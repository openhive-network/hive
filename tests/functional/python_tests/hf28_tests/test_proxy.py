...
@run_for("testnet")
def test_vote_power_value_after_proxy_remove(node):
    wallet = tt.Wallet(attach_to=node)
...
def test_proxy_change(node):
    ...
    wallet.create_account("alice", vests=tt.Asset.Test(1))
    wallet.create_account("bob", vests=tt.Asset.Test(22))
    wallet.create_account("carol", vests=tt.Asset.Test(33))
...
@run_for("testnet")
def test_set_the_proxy_twice_on_the_same_account(node):
    wallet = tt.Wallet(attach_to=node)
    ...
    for num in range(0, len(accounts), step):
        ...
        with wallet.in_single_transaction():
            for n, account in enumerate(accounts[num: num + step]):
                if account.name != accounts[-1].name:
...
def test_vesting_shares_values_on_four_proxy_layers(node):
    ...
    with wallet.in_single_transaction():
        wallet.api.set_voting_proxy("account-0", f"account-1")
        wallet.api.set_voting_proxy("account-1", f"account-2")
        wallet.api.set_voting_proxy("account-2", f"account-3")
        wallet.api.set_voting_proxy("account-3", f"account-4")
    ...
    for account_number in range(len(accounts) - 1):
        ...
        vs_position = node.api.wallet_bridge.get_account(f"account-{account_number + 1}")["proxied_vsf_votes"][0]
        assert vs_amount == vs_position
