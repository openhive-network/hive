...
def test_vote_for_comment_with_vests_from_delegation_before_decline_voting_rights(node):
    ...
    assert node.api.wallet_bridge.get_accounts(["bob"])[0]["reward_vesting_balance"] > tt.Asset.Vest(0)
    assert node.api.wallet_bridge.get_accounts(["bob"])[0]["reward_vesting_hive"] > tt.Asset.Test(0)
