...
@pytest.mark.parametrize("status", ["active", "inactive", "expired", "votable", "all"])
def test_proposal_status_filter_on_list_proposal_votes(node, api, status):
    ...
    approve_all_created_proposals(node, wallet, api, status)

    if status == "expired":
        node.wait_number_of_blocks(10)  # wait to be sure if proposal expires (10*3s = 30s)
...
def approve_all_created_proposals(node: tt.InitNode, wallet: tt.Wallet, api: str, status: str) -> None:
...
def create_account_and_proposal(wallet: tt.Wallet, account_name: str, proposal_start_date: str, proposal_end_date: str) -> None:
