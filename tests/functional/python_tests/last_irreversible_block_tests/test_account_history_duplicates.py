...
def test_account_history_duplicates():
    with World() as world:
        ...
        prepare_network(all_witness_names, api_node, init_node, wallet)

        # Test no duplicates after fork
        make_fork(alpha_net, alpha_node, beta_net, beta_node, wallet)
        assert_no_duplicates(alpha_node, api_node, beta_api_node, beta_wallet, wallet)
        ...
        assert_no_duplicates_2(api_node, wallet)
        assert_only_virtual_are_returned(api_node, wallet)


def assert_only_virtual_are_returned(api_node, wallet):
    ...
    for _ in range(10):
        ...
        response = api_node.api.account_history.enum_virtual_ops(block_range_begin=block_to_check,
                                                                 block_range_end=block_to_check + 1,
...
def assert_no_duplicates_2(api_node, wallet):
    ...
    for _ in range(10):
        ...
        api_node_reward_operations = count_ops_by_type(api_node, 'producer_reward_operation', alpha_irreversible,
                                                       limit=50)
...
def assert_no_duplicates(alpha_node, api_node, beta_api_node, beta_wallet, wallet):
    ...
    for _ in range(10):
        ...
        beta_reward_operations = count_ops_by_type(beta_api_node, 'producer_reward_operation', beta_irreversible,
                                                   limit=50)
...
def make_fork(alpha_net, alpha_node, beta_net, beta_node, wallet):
    ...
    for node in [alpha_node, beta_node]:
        while True:
            current_irreversible_block = node.api.database.get_dynamic_global_properties()["result"][
...
def prepare_network(all_witness_names, api_node, init_node, wallet):
