...
def test_enum_virtual_ops(prepare_basic_networks):
    ...
    for i in range(112, 121):
        expected_results.append(
            {"block": i, "ops": ["transfer_to_vesting_completed_operation", "producer_reward_operation"]}
        )
...
def assert_only_virtual_operations(node, block_to_check, expected_results):
    response = node.api.account_history.enum_virtual_ops(
        block_range_begin=block_to_check, block_range_end=block_to_check + 1, include_reversible=True
    )
    ops = response["ops"]
