...
def test_recurrent_transfer(wallet):
    ...
    check_recurrence_transfer(
        _result[0], "alice", "bob", tt.Asset.Test(20), "banana-cherry", 24, "remaining_executions", 2
    )
    ...
    assert len(_result) == 1
    check_recurrence_transfer(
        _result[0], "bob", "alice", tt.Asset.Test(0.9), "banana-lime", 25, "remaining_executions", 1
    )
    ...
    check_recurrence_transfer(
        _result[1], "bob", "initminer", tt.Asset.Test(0.8), "banana-lemon", 26, "remaining_executions", 21
    )
