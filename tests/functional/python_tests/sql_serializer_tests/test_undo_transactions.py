...
def test_undo_transactions(world):
    #GIVEN
    ...
    with engine.connect() as database_under_test:
        ...
        for block in range(fork_block, after_fork_block):
            logger.info(f"checking that transactions in block {block} were reverted after switching fork")
            result = database_under_test.execute(f'select * from hive.transactions where block_num={block}')
            _exhausted = object()
