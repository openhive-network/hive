...
def make_fork(world, main_chain_ops=[], fork_chain_ops=[]):
    ...
    for op in main_chain_ops:
        op.send(main_chain_wallet)
    for op in fork_chain_ops:
        op.send(fork_chain_wallet)
...
class Operation:
    def __init__(self, *args, raw_op=None):
        self.__body = None
        if raw_op:
            self.__body = json.loads(raw_op.body)
        else:
            method, *args = args
    ...
    def get_type(self):
        if self.__body:
            return self.__body["type"]
        else:
            return self.__method + '_operation'


    def send(self, wallet):
        try:
            callable = getattr(wallet.api, self.__method)
            callable(*self.__args)
        except AttributeError:
            logger.error(f'No method {self.__method} is possible to call on wallet api')
