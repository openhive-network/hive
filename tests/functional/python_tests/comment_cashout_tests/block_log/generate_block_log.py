...
def prepare_blocklog_network():
    ...
    - 1 InitNode,
    - 4 FullApiNodes,
    - 3 WitnessNodes (one witness per node).
    """
...
def prepare_blocklog_with_comments_and_votes():
    ...
    init_wallet.api.set_transaction_expiration(1024)
    ...
    init_node.wait_number_of_blocks(43)
    ...
    execute_function_in_threads(
        ...
        args=(init_wallet, __create_and_fund_voters, None),
...
def __generate_and_broadcast(wallet: tt.Wallet, func, comment_number: int | None, account_names: List[str]) -> None:
...
def __create_and_fund_voters(voter: str, vest_amount: tt.Asset.Test = tt.Asset.Test(0.1)) -> list:
