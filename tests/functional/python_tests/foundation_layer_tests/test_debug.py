...
from hive_local_tools.functional.python.datagen import VestPrice
...
def test_debug(node):
    ...
    node.wait_number_of_blocks(1)
    tt.logger.info(f"total_vesting_shares: {node.api.database.get_dynamic_global_properties()['total_vesting_shares']}")
    tt.logger.info(f"total_vesting_fund_hive: {node.api.database.get_dynamic_global_properties()['total_vesting_fund_hive']}")
