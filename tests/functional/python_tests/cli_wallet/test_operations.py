...
def test_delayed_voting(wallet: Wallet, funded_account : funded_account_info, creator : Account):
  ...
  wallet.api.transfer_to_vesting(from_=creator.name, to=account.name, amount=Asset.Test(1))
  votes_after = count_votes()
  
  assert votes_after > votes_before

def test_get_open_orders(wallet : Wallet, funded_account : funded_account_info):
  user = funded_account.account
  AMOUNT_TO_SELL_1 = Asset.Test(10)
  MIN_TO_RECEIVE_1 = Asset.Tbd(1000)
