...
def remote_node_wallet():
    paths_to_executables.set_path_of('cli_wallet',
                                     '/home/dev/ComparationHF25HF26Mainnet/src/hive_HF25/build/programs/cli_wallet/cli_wallet')
    remote_node = RemoteNode(http_endpoint='0.0.0.0:19091', ws_endpoint='0.0.0.0:19090')
...
def test_gethelp(remote_node_wallet, cli_wallet_method, arguments):
    ...
    with open(f'output_files_hf25_gethelp/{cli_wallet_method}.json', 'w') as json_file:
        json.dump(response, json_file)
        json_file.close()
...
def test_gethelp(remote_node_wallet, operation):
    ...
    with open(f'output_files_hf25_get_prototype_operation/{operation}.json', 'w') as json_file:
        json.dump(response, json_file)
        json_file.close()
