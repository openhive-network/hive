...
METHODS = [
    ...
    ('get_encrypted_memo', ()), # no work, less of operation of memo
    ('get_prototype_operation', ('account_create_operation',)),   # work
    ('list_proposals', ([""], 100, 29, 0, 0)),   # work but return nothing
    ('find_proposals', ()),  # work but return nothing ( there is not proposals
...
METHODS = [
    ...
    # ('get_private_key', ()),  # unknown key, nie działa w block logu 5m
    # ('get_private_key_from_password', ()),  # unknown key, nie działa w block logu 5m
    ('list_my_accounts', ()),  # work but return nothing ( there no acccount created with this wallet)
    ('get_conversion_requests', ('gtg',)),  # work but return nothing
    ('get_collateralized_conversion_requests', ('gtg',)),  # work but return nothing
    ('get_withdraw_routes', ('gtg', 'all')),  # work but return nothing
    ...
    # ('list_proposals', ([""], 100, 29, 0, 0)),  # nie działa w HF25
    ('find_proposals', ([0],)),  # work but return nothing ( there is not proposals
    # ('list_proposal_votes', ([""], 100, 33, 0, 0)),  # nie działa w HF25
    # ('info', ()),  # nie działa w HF25
]
...
def test_compare_methods(cli_wallet_method, arguments):
    with open(f'output_files_hf25/{cli_wallet_method}.json', 'r') as json_file:
        response_hf25 = json_file.read()
        json_file.close()

    with open(f'output_files_hf26/{cli_wallet_method}.json', 'r') as json_file:
        response_hf26 = json_file.read()
        json_file.close()
...
def test_compare_methods(cli_wallet_method, arguments):
    with open(f'output_files_hf25_gethelp/{cli_wallet_method}.json', 'r') as json_file:
        response_hf25 = json_file.read()
        json_file.close()

    with open(f'output_files_hf26_gethelp/{cli_wallet_method}.json', 'r') as json_file:
        response_hf26 = json_file.read()
        json_file.close()
