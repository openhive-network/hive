...
from test_tools.__private.communication import LegacyJsonEncoderTestnet
...
def send_rpc_query(target_node: str, payload: dict) -> dict:
    ...
    resp = post(target_node, data=dumps(payload, cls=LegacyJsonEncoderTestnet))
...
def get_current_block_number(source_node) -> int:
    ...
    try:
        resp = post(source_node, data=dumps(payload, cls=LegacyJsonEncoderTestnet))
        if resp.status_code != 200:
            return -1
