...
class _RcManabar(_BaseManabar):
    ...
    def assert_rc_current_mana_is_reduced(
        self, operation_rc_cost: int, operation_timestamp: datetime | None = None, tolerance_percent: float = 0.02
    ) -> None:
        """
        ...
        Args:
            ...
            tolerance_percent: Allowed tolerance as a fraction (default 2% to account for mana regeneration)
        """
    ...
    def assert_current_mana_is_unchanged(self, tolerance_percent: float = 0.01) -> None:
        """
        ...
        tolerance = max(int(self.current_mana * tolerance_percent), 1000)
