...
def convert_hbd_to_hive(node, hbd: tt.Asset.Hbd) -> tt.Asset.Hive:
    ...
    return AssetHive(
        amount=calculated_hive.amount.decode(),
        precision=calculated_hive.precision,
        nai=calculated_hive.nai.decode(),
    )
...
def convert_vesting_to_hive(node, vesting: tt.Asset.Vest) -> tt.Asset.Hive:
    ...
    return AssetHive(
        amount=calculated_hp.amount.decode(),
        precision=calculated_hp.precision,
        nai=calculated_hp.nai.decode(),
    )
