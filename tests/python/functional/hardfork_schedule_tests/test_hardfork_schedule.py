...
def test_simply_hardfork_schedule() -> None:
    ...
    """
    # The number of witnesses (16 + 1) is chosen because it is the required threshold to vote for hardfork activation.
    witnesses_required_for_hf06_and_later = [f"witness-{witness_num}" for witness_num in range(16)]

    init_node = tt.InitNode()

    # The queen plugin is used to stabilize the test on CI.
    # The initial version of the test was unstable under CI load, frequently causing "missing block" events.
    # These missing blocks prevented hardforks from activating at the expected moments, what caused the test to fail
