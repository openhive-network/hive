...
def test_get_block_range_crash(replayed_node: tt.ApiNode) -> None:
    response = replayed_node.api.block.get_block_range(starting_block_num=60, count=30)
    assert len(response.blocks) == 30


def test_get_block_range(replayed_node: tt.ApiNode) -> None:
    step = 100
...
def test_get_block_range_out_of_range(replayed_node: tt.ApiNode) -> None:
    step = 100
