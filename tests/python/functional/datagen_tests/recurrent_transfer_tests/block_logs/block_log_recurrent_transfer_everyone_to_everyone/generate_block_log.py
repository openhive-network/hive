...
def prepare_block_log(output_block_log_directory: Path) -> None:
    ...
    execute_function_in_threads(
        ...
        args_sequences=(account_names,),
        kwargs={"transfer_asset": tt.Asset.Test(int(liquid_per_account) // 1000), "legacy_mode": True},
        amount=AMOUNT_OF_ALL_ACCOUNTS,
        chunk_size=ACCOUNTS_PER_CHUNK,
        max_workers=MAX_WORKERS,
    )
    ...
    execute_function_in_threads(
        ...
        args_sequences=(account_names,),
        kwargs={"vests_amount": (vs_per_account - tt.Asset.Vest(10)).as_legacy(), "legacy_mode": True},
        amount=AMOUNT_OF_ALL_ACCOUNTS,
        chunk_size=ACCOUNTS_PER_CHUNK,
        max_workers=MAX_WORKERS,
    )
    ...
    execute_function_in_threads(
        ...
        args_sequences=(account_names,),
        kwargs={"amount": AssetHiveHF26(amount=1), "executions": 2, "recurrence": 24},
        amount=AMOUNT_OF_ALL_ACCOUNTS,
        chunk_size=ACCOUNTS_PER_CHUNK,
        max_workers=MAX_WORKERS,
    )
    generate_block(node, 10)
