...
@run_for("testnet")
def test_exceed_date_limit_of_recurrent_transfers(node: tt.InitNode, executions: int) -> None:
    """
...
@run_for("testnet")
def test_last_execution_of_recurrent_transfer_close_to_date_limit(node: tt.InitNode, executions: int) -> None:
    """
    ...
    assert node.api.condenser.get_accounts(["sender"])[0].get_balance() == "100.000 TESTS"
    # Validate that receiver's balance is equal 0 Tests
    assert node.api.condenser.get_accounts(["receiver"])[0].get_balance() == "0.000 TESTS"
    ...
    assert node.api.condenser.get_accounts(["sender"])[0].get_balance() == "90.000 TESTS"
    # Validate that receiver's balance is equal 10 Tests
    assert node.api.condenser.get_accounts(["receiver"])[0].get_balance() == "10.000 TESTS"
