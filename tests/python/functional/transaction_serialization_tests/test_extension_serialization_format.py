...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [0, {"beneficiaries": [{"account": "bob", "weight": 100}]}],
        ),
        (
            "block",
...
def test_update_proposal_operation(node, api, expected_extension):
    ...
    wallet.api.create_proposal(
        "alice", "alice", PROPOSAL_START_DATE, PROPOSAL_END_DATE, tt.Asset.Tbd(100), "subject-1", "permlink"
    )
    operation = wallet.api.update_proposal(
        0, "alice", tt.Asset.Tbd(10), "subject-1", "permlink", PROPOSAL_END_DATE_AFTER_UPDATE
    )
