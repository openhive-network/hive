...
def test_handling_sensitive_data_in_the_memo_field(
    ...
    error_message = "#"
    if memo_type == "private_key":
        with pytest.raises(PrivateKeyInMemoError):
...
@run_for("testnet")
@pytest.mark.parametrize("operation", ["transfer"])
def test_handle_by_wallet_additional_private_key_in_memo_field(
    ...
    with pytest.raises(PrivateKeyInMemoError) as error:
        broadcast_transaction_by_wallet(wallet, operation, memo=extra_private_key)

    assert "Detected imported private key in memo field. Cancelling transaction." in str(error.value.args)
