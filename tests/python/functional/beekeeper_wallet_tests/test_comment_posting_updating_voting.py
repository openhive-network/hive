...
def test_comment(wallet: tt.Wallet) -> None:
    ...
    response = wallet.api.post_comment(  # hasz23
        "alice",
        "test-permlink",
        "",
        "xyz",
    ...
    assert _ops[0].type == "comment_operation"
    ...
    _ops = response.operations
    assert _ops[0].type == "vote_operation"
    ...
    assert _ops[0].type == "decline_voting_rights_operation"
