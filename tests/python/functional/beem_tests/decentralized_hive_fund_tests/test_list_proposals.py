...
def list_proposals_test(node_client, creator):
    ...
    assert proposals[0]["subject"] == START_END_SUBJECTS[0][2], (
        "Subject of the first proposal does not match with assumed proposal subject"
        f" {proposals[0]['subject']}!={START_END_SUBJECTS[0][2]}"
    )
    assert proposals[-1]["subject"] == START_END_SUBJECTS[-1][2], (
        "Subject of the last proposal does not match with assumed proposal subject"
        f" {proposals[-1]['subject']}!={START_END_SUBJECTS[-1][2]}"
    )
    ...
    assert proposals[0]["subject"] == START_END_SUBJECTS[-1][2], (
        "Subject of the first proposal does not match with assumed proposal subject"
        f" {proposals[0]['subject']}!={START_END_SUBJECTS[-1][2]}"
    )
    assert proposals[-1]["subject"] == START_END_SUBJECTS[0][2], (
        "Subject of the last proposal does not match with assumed proposal subject"
        f" {proposals[-1]['subject']}!={START_END_SUBJECTS[0][2]}"
    )
    ...
    assert proposals[0]["subject"] == START_END_SUBJECTS[0][2], (
        "Subject of the proposal does not match with assumed proposal subject"
        f" {proposals[0]['subject']}!={START_END_SUBJECTS[0][2]}"
    )
