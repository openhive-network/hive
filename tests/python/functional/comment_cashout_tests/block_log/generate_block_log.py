...
def prepare_blocklog_with_comments_and_votes():
    ...
    execute_function_in_threads(
        ...
        args=(init_wallet, init_node, __create_and_fund_voter, None),
...
def generate_block(node: tt.InitNode, number: int) -> None:
    node.api.debug_node.debug_generate_blocks(
        # TODO: Andrzej ma naprawić , obejściem jest nie podawanie klucza
        debug_key="",
        count=number,
        skip=0,
        miss_blocks=0,
...
def create_comments(node: tt.InitNode, wallet: tt.Wallet) -> None:
    ...
    # accounts = wallet.create_accounts(number_of_accounts=AMOUNT_OF_ALL_COMMENTS, name_base="creator", broadcast=False)
...
def __generate_and_broadcast_transaction(
    wallet: tt.Wallet, node: tt.InitNode, func, comment_number: int | None, account_names: list[str]
) -> None:
...
def __create_and_fund_voter(voter: str) -> AccountCreateOperation:
