...
def test_key_import(beekeeper: Beekeeper, prv_pub: tuple[str, str], wallet: WalletInfo) -> None:
    # ARRANGE & ACT
    pub_key = (beekeeper.api.beekeeper.import_key(wallet_name=wallet.name, wif_key=prv_pub[0])).public_key

    # ASSERT
    assert_keys([pub_key], [prv_pub[1]])
    assert_keys(beekeeper.api.beekeeper.get_public_keys().keys, [prv_pub[1]])


def test_import_multiple_keys(beekeeper: Beekeeper, wallet: WalletInfo) -> None:
    # ARRANGE & ACT
    import_keys_result: list[ImportKey] = [
        beekeeper.api.beekeeper.import_key(wallet_name=wallet.name, wif_key=prv) for prv, _ in PRIVATE_AND_PUBLIC_KEYS
    ]
    public_keys = [x.public_key for x in import_keys_result]
    received_public_keys = beekeeper.api.beekeeper.get_public_keys().keys
...
def test_remove_key(
    ...
    # ACT & ASSERT
    assert_keys((beekeeper.api.beekeeper.get_public_keys()).keys, [public_key])
    beekeeper.api.beekeeper.remove_key(
        wallet_name=wallet.name,
        password=wallet.password,
        public_key=public_key,
    )
    assert_keys((beekeeper.api.beekeeper.get_public_keys()).keys)
