...
def test_api_close_session(beekeeper: Beekeeper) -> None:
    ...
    # ARRANGE & ACT
    beekeeper.api.beekeeper.close_session()

    # ASSERT
    close_log_entry = f'"id":0,"jsonrpc":"2.0","method":"beekeeper_api.close_session","params":{{"token":"{beekeeper.session_token}"}}'
    with pytest.raises(RequestError, match=f"A session attached to {beekeeper.session_token} doesn't exist"):
        beekeeper.api.beekeeper.close_session()
...
def test_if_beekeeper_closes_after_last_session_termination(beekeeper: Beekeeper) -> None:
...
def test_api_close_session_not_existing(create_session: bool, beekeeper: Beekeeper) -> None:
    ...
    # ACT & ASSERT
    beekeeper._SessionHolder__session_token = WRONG_TOKEN  # noqa: SLF001  # this is only for test purposes
