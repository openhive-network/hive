...
def test_api_close(beekeeper: Beekeeper, wallet: WalletInfo) -> None:
    ...
    # ARRANGE
    assert len((beekeeper.api.beekeeper.list_wallets()).wallets) == 1
    ...
    # ASSERT
    assert (
        len((beekeeper.api.beekeeper.list_wallets()).wallets) == 0
...
def test_api_close_import_key_to_closed_wallet(
    ...
    with pytest.raises(RequestError, match=f"Wallet not found: {wallet.name}"):
        beekeeper.api.beekeeper.import_key(wif_key=keys_to_import[0].private_key, wallet_name=wallet.name)
