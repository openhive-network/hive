...
def test_obi_throw_exception_01(prepare_obi_throw_exception_01):
    ...
    ttcn.wait_for_specific_witnesses(witness_node_0, logs, [["witness-0", "initminer"], ["witness-1"]])
    ...
    ttcn.wait_for_specific_witnesses(witness_node_1, logs, [["witness-1"]])
    ...
    assert ttcn.get_last_head_block_number(_a0) == ttcn.get_last_head_block_number(_a1)
    assert ttcn.get_last_head_block_number(_w0) < ttcn.get_last_head_block_number(_w1)
    assert ttcn.get_last_head_block_number(_a0) > ttcn.get_last_head_block_number(_w0)

    assert ttcn.get_last_irreversible_block_num(_a0) == ttcn.get_last_irreversible_block_num(_a1)
    assert ttcn.get_last_irreversible_block_num(_w0) == ttcn.get_last_irreversible_block_num(_w1)
    assert ttcn.get_last_irreversible_block_num(_a0) == ttcn.get_last_irreversible_block_num(_w0)
