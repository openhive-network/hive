...
def test_2_1_case(prepared_node, wallet):
    ...
    for account in [user_a, user_b, user_c]:
        account.assert_reward_hbd_balance(prepared_node)
        account.assert_reward_vesting_balance(prepared_node)
    ...
    user_c.assert_curation_reward_virtual_operation("not_generated")  # 3.

    # 44. User does not receive the author reward for the comment.
    reply_1.assert_author_reward_virtual_operation("not_generated")  # 2.

    # 43. User does not receive the curation reward for the comment.
...
def test_2_2_case(prepared_node, wallet):
    ...
    post_1.assert_resource_percentage_in_author_reward(hbd_percentage=50, vesting_percentage=50)  # 1.
    ...
    reply_1.assert_resource_percentage_in_author_reward(hbd_percentage=50, vesting_percentage=50)  # 1.
...
def test_2_3_case(prepared_node, wallet):
    ...
    # 57. Users who downvote the post don't receive the curation reward.
    user_c.assert_curation_reward_virtual_operation("not_generated")  # 2.
    user_e.assert_curation_reward_virtual_operation("not_generated")  # 4.
