...
def recurrent_transfer_setup(request, node, wallet, sender, receiver):
    ...
    for iteration in range(3):
        recurrent_transfers.append(RecurrentTransfer(node, wallet,
                                                     from_=sender.name,
                                                     to=receiver.name,
        ...
        recurrent_transfers[iteration].assert_fill_recurrent_transfer_operation_was_generated(expected_vop=iteration+1)
        sender.update_account_info()
        receiver.update_account_info()
        if iteration == 0:
            jump_to_date(node, recurrent_transfers[iteration].timestamp +
