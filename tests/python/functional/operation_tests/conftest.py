...
from schemas.operations.witness_block_approve_operation import (
...
class ConvertAccount(Account):
    ...
    @staticmethod
    def extract_amount_from_convert_operation(
        transaction: dict,
    ) -> tt.Asset.HiveT | tt.Asset.TbdT:
...
class EscrowAccount(Account):
    ...
    def check_account_balance(
        ...
        trx: dict,
        mode: Literal[
            "escrow_creation_operation",
            "escrow_rejection_operation",
...
class UpdateAccount(Account):
    ...
    def generate_new_authority(self) -> dict:
        return {
            "account_auths": [],
            "key_auths": [(self.generate_new_key(), 1)],
            "weight_threshold": 1,
        }
...
class WitnessAccount(Account):
    ...
    def become_witness(
        self,
        url: str,
        account_creation_fee: tt.Asset.Test,
    ...
    def witness_block_approve(self, *, block_id: int) -> dict:
        return create_transaction_with_any_operation(
            self._wallet,
            [WitnessBlockApproveOperation(witness=self._name, block_id=block_id)],
        )
