...
def test_cancel_transfer_from_savings_simplest_scenario(
    ...
    currency: type[tt.Asset.TestT | tt.Asset.TbdT],
    check_savings_balance: str,
    check_balance: str,
) -> None:
    asset_50 = currency(amount=50)
    alice.transfer_to_savings("alice", asset_50, "transfer to savings")
    ...
    alice.transfer_from_savings(0, "alice", asset_50, "transfer from savings")

    rc_amount_after_sending_op = alice.get_rc_current_mana()
    balance_after_withdrawal = getattr(alice, check_savings_balance)()

    assert (
        balance_before_withdrawal == balance_after_withdrawal + asset_50
    ), f"{currency.get_asset_information().get_symbol()} savings balance wasn't decreased after withdrawal"
    ...
    assert (
        savings_balance_before_withdrawal_cancellation + asset_50 == savings_balance_after_withdrawal_cancellation
    ), f"{currency.get_asset_information().get_symbol()}S from cancelled withdrawal didn't come back to savings balance"
    ...
    assert getattr(alice, check_balance)() - funds_after_transfer_to_savings == currency(amount=0), (
        f"{currency.get_asset_information().get_symbol()}S from cancelled withdrawal arrived to receiver account"
        f" {currency.get_asset_information().get_symbol()} balance"
    )
...
def test_cancel_all_transfers_from_savings(
    ...
    assert getattr(alice, check_balance)() - funds_after_transfer_to_savings == currency(amount=0), (
        "At least one of cancelled withdrawals arrived to receiver account"
        f" {currency.get_asset_information().get_symbol()} balance"
    )
...
def test_cancel_all_transfers_from_savings_except_one(
    ...
    assert getattr(alice, check_balance)() - funds_after_transfer_to_savings == currency(amount=10), (
        "At least one of cancelled withdrawals arrived to receiver account"
        f" {currency.get_asset_information().get_symbol()} balance or the withdrawal that wasn't cancelled didn't"
        " arrive."
    )
...
def create_three_savings_withdrawals_from_fresh_account(
    ...
    for time_offset in (6, 12, 18):
        ...
        ), f"RC amount after {currency.get_asset_information().get_symbol()} withdrawal wasn't decreased."
        assert (
