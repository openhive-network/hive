...
def send_transfers_to_vesting_from_initminer_to_alice(wallet: tt.Wallet, *, amount: int) -> None:
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_find_irreversible_operations(node: tt.InitNode) -> None:
    block_to_start = node.get_last_block_number()
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_find_newly_created_virtual_op(node: tt.InitNode) -> None:
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_find_reversible_virtual_operations(node: tt.InitNode) -> None:
    block_to_start = node.get_last_block_number()
    node.wait_number_of_blocks(1)
    response = node.api.account_history.enum_virtual_ops(
        block_range_begin=block_to_start,
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_grouping_by_block(node: tt.InitNode) -> None:
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_limit(node: tt.InitNode, group_by_block: bool, include_reversible: bool) -> None:
    node.wait_number_of_blocks(1)
    block_to_start = node.get_last_block_number()
    wallet = tt.Wallet(attach_to=node)
...
def test_list_vops_partly_in_irreversible_and_partly_in_reversible_blocks(
    node: tt.InitNode, group_by_block: bool, key: str
) -> None:
    node.wait_number_of_blocks(1)
    block_to_start = node.get_last_block_number()
    wallet = tt.Wallet(attach_to=node)
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_no_virtual_operations(node: tt.InitNode, group_by_block: bool) -> None:
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_number_of_producer_reward_ops(node: tt.InitNode) -> None:
    node.wait_number_of_blocks(3)
    block_to_start = node.get_last_block_number()
    blocks_to_wait = 5
...
@run_for("testnet", enable_plugins=["account_history_api"])
def test_pagination(node: tt.InitNode) -> None:
    node.wait_number_of_blocks(1)
    block_to_start = node.get_last_block_number()
    wallet = tt.Wallet(attach_to=node)
