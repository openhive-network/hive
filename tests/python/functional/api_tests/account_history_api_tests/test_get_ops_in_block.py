...
def test_default_args_value(node: tt.InitNode) -> None:
...
def test_filter_virtual_ops(node: tt.InitNode, only_virtual: bool, number_of_ops: int) -> None:
    ...
    response = node.api.account_history.get_ops_in_block(
        block_num=block_number, only_virtual=only_virtual, include_reversible=True
    )
...
def test_get_operations_in_block_with_and_without_reversible(
    node: tt.InitNode, include_reversible: bool, comparison_type: str
) -> None:
    response = node.api.account_history.get_ops_in_block(
        block_num=1, only_virtual=False, include_reversible=include_reversible
    )
    assert getattr(len(response.ops), comparison_type)(0)


@run_for("testnet", enable_plugins=["account_history_api"])
def test_get_ops_in_non_existent_block(node: tt.InitNode) -> None:
