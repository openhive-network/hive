...
def test_get_order_book_with_different_values(
    ...
    assert convert_to_asset(response.asks[0].order_price.base) == tt.Asset.Test(ask_hive_amount)
    assert convert_to_asset(response.asks[0].order_price.quote) == tt.Asset.Tbd(ask_hbd_amount)
    assert float(response.asks[0].real_price) == ask_hbd_amount / ask_hive_amount

    assert convert_to_asset(response.bids[0].order_price.base) == tt.Asset.Tbd(bid_hbd_amount)
    assert convert_to_asset(response.bids[0].order_price.quote) == tt.Asset.Test(bid_hive_amount)
