...
def test_recent_trades_output_parameters(node: tt.InitNode, limit_orders: dict) -> None:
    ...
    for number in range(len(limit_orders)):
        ...
        wallet.api.create_order(
            "alice",
            number,
            tt.Asset.Test(limit_orders[order]["tests"]),
            tt.Asset.Tbd(limit_orders[order]["tbds"]),
            False,
            3600,
        )
        wallet.api.create_order(
    ...
    for x in range(len(response)):
        ...
        assert tt.Asset.Tbd(limit_orders["order_" + str(x)]["tbds"]) == convert_to_asset(trade.current_pays)
        assert tt.Asset.Test(limit_orders["order_" + str(x)]["tests"]) == convert_to_asset(trade.open_pays)
