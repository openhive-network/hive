...
@pytest.fixture()
def unconfigured_offline_wallet() -> tt.Wallet:
    return tt.Wallet(preconfigure=False)


@pytest.fixture()
def unconfigured_online_wallet(node) -> tt.Wallet:
    return tt.Wallet(attach_to=node, preconfigure=False)


@pytest.fixture()
def configured_offline_wallet() -> tt.Wallet:
    return tt.Wallet()


@pytest.fixture(params=["unconfigured_online_wallet", "unconfigured_offline_wallet"])
def unconfigured_wallet(request) -> tt.Wallet:
    return request.getfixturevalue(request.param)


@pytest.fixture(params=["wallet", "configured_offline_wallet"])
def configured_wallet(request) -> tt.Wallet:
    return request.getfixturevalue(request.param)


def test_if_state_is_new_after_first_start(unconfigured_wallet: tt.Wallet) -> None:
    assert unconfigured_wallet.api.is_new() is True
    assert unconfigured_wallet.api.is_locked() is True


def test_if_state_is_locked_after_first_password_set(unconfigured_wallet: tt.Wallet) -> None:
    unconfigured_wallet.api.set_password(unconfigured_wallet.DEFAULT_PASSWORD)
    assert unconfigured_wallet.api.is_new() is False
    assert unconfigured_wallet.api.is_locked() is True


def test_if_state_is_unlocked_after_entering_password(unconfigured_wallet: tt.Wallet) -> None:
    unconfigured_wallet.api.set_password(unconfigured_wallet.DEFAULT_PASSWORD)
...
def test_if_state_is_locked_after_entering_password(unconfigured_wallet: tt.Wallet) -> None:
    unconfigured_wallet.api.set_password(unconfigured_wallet.DEFAULT_PASSWORD)
    unconfigured_wallet.api.unlock(unconfigured_wallet.DEFAULT_PASSWORD)
...
def test_if_state_is_locked_after_close_and_reopen(unconfigured_wallet: tt.Wallet) -> None:
    unconfigured_wallet.api.set_password(unconfigured_wallet.DEFAULT_PASSWORD)
    unconfigured_wallet.api.unlock(unconfigured_wallet.DEFAULT_PASSWORD)
...
def test_save_wallet_to_file(configured_wallet: tt.Wallet) -> None:
...
def test_import_keys(unconfigured_wallet: tt.Wallet) -> None:
    unconfigured_wallet.api.set_password(unconfigured_wallet.DEFAULT_PASSWORD)
    unconfigured_wallet.api.unlock(unconfigured_wallet.DEFAULT_PASSWORD)
    keys = unconfigured_wallet.api.list_keys()
    assert len(keys) == 0
...
def test_generate_keys(configured_wallet: tt.Wallet) -> None:
...
def test_get_private_key_related_to_public_key(configured_wallet: tt.Wallet) -> None:
...
def test_suggest_brain_key(configured_wallet: tt.Wallet) -> None:
...
def test_serialize_transaction(configured_wallet: tt.Wallet, node: tt.InitNode) -> None:
...
def test_get_encrypted_memo_and_decrypt_memo(configured_wallet: tt.Wallet, node: tt.InitNode) -> None:
    wallet_temp = tt.Wallet(attach_to=node)
...
def test_exit_from_wallet(configured_wallet: tt.Wallet) -> None:
