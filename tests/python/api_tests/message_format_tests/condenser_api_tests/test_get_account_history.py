...
def test_get_account_history(node: tt.InitNode | tt.RemoteNode, should_prepare: bool) -> None:
    preparation_for_testnet_node(node, should_prepare)
    node.api.condenser.get_account_history("alice", -1, 10, 5, 10)


@run_for("testnet", "mainnet_5m", "live_mainnet", enable_plugins=["account_history_api"])
def test_get_account_history_with_default_fifth_argument(
...
@run_for("testnet", "mainnet_5m", "live_mainnet", enable_plugins=["account_history_api"])
def test_get_account_history_with_default_fourth_and_fifth_argument(
    node: tt.InitNode | tt.RemoteNode, should_prepare: bool
) -> None:
    preparation_for_testnet_node(node, should_prepare)
