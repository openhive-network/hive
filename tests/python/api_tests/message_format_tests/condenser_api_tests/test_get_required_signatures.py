...
def test_get_required_signatures_in_testnet(node: tt.InitNode) -> None:
    ...
    block = node.api.condenser.get_block(transaction["block_num"])
    account = node.api.wallet_bridge.get_accounts(["initminer"])[0]
    transaction2 = block.transactions[0]
    active_key = account.active.key_auths[0][0]
    keys = node.api.condenser.get_required_signatures(transaction2, [active_key])
    assert len(keys) != 0
