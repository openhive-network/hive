...
def test_find_vesting_delegations(node: tt.InitNode | tt.RemoteNode, should_prepare: bool) -> None:
    if should_prepare:
    ...
    account = (
        node.api.database.list_vesting_delegations(start=["catharsis", "veshu1230"], limit=100, order="by_delegation")
        .delegations[0]
        .delegator
    )
