...
def test_find_owner_histories(node: tt.InitNode | tt.RemoteNode, should_prepare: bool) -> None:
    if should_prepare:
    ...
    account = (
        node.api.database.list_owner_histories(start=["", tt.Time.from_now(weeks=-100)], limit=100)
        .owner_auths[0]
        .account
    )
