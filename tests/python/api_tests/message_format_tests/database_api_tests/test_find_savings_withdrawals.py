...
def test_find_savings_withdrawals(node: tt.InitNode | tt.RemoteNode, should_prepare: bool) -> None:
    if should_prepare:
        wallet = tt.Wallet(attach_to=node)
        wallet.create_account("alice", hives=tt.Asset.Test(100), vests=tt.Asset.Test(100))
        transfer_and_withdraw_from_savings(wallet, "alice")
    account = (
        node.api.database.list_savings_withdrawals(
            start=[tt.Time.from_now(weeks=-100), "", 0], limit=100, order="by_complete_from_id"
        )
        .withdrawals[0]
        .to
    )
