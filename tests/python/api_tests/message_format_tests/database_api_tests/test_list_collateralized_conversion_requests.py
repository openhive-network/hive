...
def test_list_collateralized_conversion_requests(node: tt.InitNode | tt.RemoteNode, should_prepare: bool) -> None:
    if should_prepare:
        wallet = tt.Wallet(attach_to=node)
        wallet.create_account("alice", hives=tt.Asset.Test(100), vests=tt.Asset.Test(100))
        wallet.api.convert_hive_with_collateral("alice", tt.Asset.Test(4))
    requests = node.api.database.list_collateralized_conversion_requests(
