...
def test_find_hbd_conversion_requests(node: tt.InitNode | tt.RemoteNode, should_prepare: bool) -> None:
    if should_prepare:
    ...
    account = (
        node.api.database.list_hbd_conversion_requests(start=["", 0], limit=100, order="by_account").requests[0].owner
    )
