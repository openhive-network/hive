...
def test_get_transaction(prepared_node, should_prepare):
    if should_prepare:
        wallet = tt.Wallet(attach_to=prepared_node)
        wallet.api.create_account('initminer', 'gtg', '{}')
    result = prepared_node.api.account_history.get_account_history(account='gtg', start=1, limit=1, include_reversible=True)
    transaction_id = result['history'][0][1]['trx_id']
