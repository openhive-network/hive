...
def test_get_rc_operation_stats_with_valid_operation_name(node, operation):
    node.api.rc.get_rc_operation_stats(operation=operation)
...
@run_for('live_mainnet')
def test_get_rc_operation_stats_with_valid_operation_number(node, operation_index):
    node.api.rc.get_rc_operation_stats(operation=operation_index)


@run_for('live_mainnet')
def test_get_rc_operation_stats_with_nonexistent_operation_name(node):
    with pytest.raises(tt.exceptions.CommunicationError):
        node.api.rc.get_rc_operation_stats(operation=INCORRECT_OPERATION_NAME)
...
def test_get_rc_operation_stats_with_nonexistent_operation_index(node, invalid_operation_index):
    with pytest.raises(tt.exceptions.CommunicationError):
        node.api.rc.get_rc_operation_stats(operation=invalid_operation_index)
