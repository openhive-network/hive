...
def test_find_savings_withdrawals(prepared_node, should_prepare):
    if should_prepare:
        wallet = tt.Wallet(attach_to=prepared_node)
        wallet.create_account('alice', hives=tt.Asset.Test(100), vests=tt.Asset.Test(100))
        transfer_and_withdraw_from_savings(wallet, 'alice')
    account = prepared_node.api.database.list_savings_withdrawals(start=[date_from_now(weeks=-100), '', 0],
                                                                  limit=100,
