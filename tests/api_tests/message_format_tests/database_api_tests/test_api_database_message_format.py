...
from hive_local_tools.api.message_format import (
    create_and_cancel_vesting_delegation,
    create_proposal,
    prepare_escrow,
    request_account_recovery,
    transfer_and_withdraw_from_savings,
)
...
def test_list_hbd_conversion_requests(wallet, node):
    wallet.create_account("alice", hives=tt.Asset.Test(100), vests=tt.Asset.Test(100), hbds=tt.Asset.Tbd(100))
    wallet.api.convert_hbd("alice", tt.Asset.Tbd(1.25))
    requests = node.api.database.list_hbd_conversion_requests(start=["alice", 0], limit=100, order="by_account")[
        "requests"
    ]
    assert len(requests) != 0
...
def test_list_owner_histories(node, wallet):
    ...
    # update_account_auth_key with owner parameter is called to change owner authority history
    wallet.api.update_account_auth_key("alice", "owner", tt.Account("some key").public_key, 1)
    owner_auths = node.api.database.list_owner_histories(start=["alice", tt.Time.from_now(weeks=-1)], limit=100)[
        "owner_auths"
    ]
    assert len(owner_auths) != 0
...
def test_list_savings_withdrawals(node, wallet):
    ...
    withdrawals = node.api.database.list_savings_withdrawals(
        start=[tt.Time.from_now(weeks=-1), "alice", 0], limit=100, order="by_complete_from_id"
    )["withdrawals"]
    assert len(withdrawals) != 0
...
def test_list_vesting_delegation_expirations(node, wallet):
    ...
    delegations = node.api.database.list_vesting_delegation_expirations(
        start=["alice", tt.Time.from_now(weeks=-1), 0], limit=100, order="by_account_expiration"
    )["delegations"]
    assert len(delegations) != 0
...
def test_list_vesting_delegations(node, wallet):
    ...
    wallet.api.create_account("alice", "bob", "{}")
    wallet.api.delegate_vesting_shares("alice", "bob", tt.Asset.Vest(5))
    delegations = node.api.database.list_vesting_delegations(start=["alice", "bob"], limit=100, order="by_delegation")[
        "delegations"
    ]
    assert len(delegations) != 0
...
def test_list_withdraw_vesting_routes(node, wallet):
    ...
    wallet.api.create_account("alice", "bob", "{}")
    wallet.api.set_withdraw_vesting_route("alice", "bob", 15, True)
    routes = node.api.database.list_withdraw_vesting_routes(
        start=["alice", "bob"], limit=100, order="by_withdraw_route"
    )["routes"]
    assert len(routes) != 0
...
def test_list_proposal_votes(node, wallet):
    ...
    proposal_votes = node.api.database.list_proposal_votes(
        start=["alice"], limit=100, order="by_voter_proposal", order_direction="ascending", status="all"
    )["proposal_votes"]
...
def test_list_proposals(node, wallet):
    wallet.create_account("alice", hives=tt.Asset.Test(100), vests=tt.Asset.Test(100), hbds=tt.Asset.Tbd(300))
    create_proposal(wallet, "alice")
    proposals = node.api.database.list_proposals(
