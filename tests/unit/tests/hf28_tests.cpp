...
BOOST_AUTO_TEST_CASE( artificial_1_on_power_down )
{
  try
  {
    ...
    HIVE_REQUIRE_ASSERT( withdraw_vesting( "eric", asset( 1, VESTS_SYMBOL ), eric_private_key ), "account.get_vesting_withdraw_rate() != new_vesting_withdraw_rate" );
    generate_block();
    ...
    try
    {
        ...
        auto calculate_expected_new_virtual_supply = [&]() -> share_type
        {
            ...
            if (db->has_hardfork(HIVE_HARDFORK_1_28_NO_DHF_HBD_IN_INFLATION)) {
              ...
              const auto hbd_supply_without_treasury = (props.get_current_hbd_supply() - treasury_assets.get_hbd_balance()).amount < 0 ? asset(0, HBD_SYMBOL) : (props.get_current_hbd_supply() - treasury_assets.get_hbd_balance());
