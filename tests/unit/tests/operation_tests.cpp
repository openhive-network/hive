...
#define GET_RECOVERY( account_name ) (db->get< recovery_object, by_account_id >( db->get_account( account_name ).get_id() ))
#define GET_RECOVERY_FOR_ACC( acc ) (db->get< recovery_object, by_account_id >( (acc).get_id() ))
...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
  try
  {
    ...
    const auto& init_assets = db->get< assets_object, by_account_id >( init.get_id() );
    ...
    const auto& acct_assets = db->get< assets_object, by_account_id >( acct.get_id() );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
  try
  {
    ...
    HIVE_REQUIRE_ASSERT( push_transaction( tx, { alice_post_key, bob_private_key } ),
      "_now - auth.get_last_post_edit() >= HIVE_MIN_COMMENT_EDIT_INTERVAL" );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
  try
  {
    ...
    const auto& _alice_mrc = db->get< manabars_rc_object, by_account_id >( _alice.get_id() );
    const auto& _bob_mrc = db->get< manabars_rc_object, by_account_id >( _bob.get_id() );
    const auto& _sam_mrc = db->get< manabars_rc_object, by_account_id >( _sam.get_id() );
    const auto& _dave_mrc = db->get< manabars_rc_object, by_account_id >( _dave.get_id() );
    const auto& _alice_assets = db->get< assets_object, by_account_id >( _alice.get_id() );
    const auto& _sam_assets = db->get< assets_object, by_account_id >( _sam.get_id() );
    const auto& _dave_assets = db->get< assets_object, by_account_id >( _dave.get_id() );
    const auto& _alice_time = db->get< time_object, by_account_id >( _alice.get_id() );
    const auto& _sam_time = db->get< time_object, by_account_id >( _sam.get_id() );
    const auto& _dave_time = db->get< time_object, by_account_id >( _dave.get_id() );
    ...
    BOOST_REQUIRE( alice_comment_cashout->get_net_rshares() == ( old_mana - _alice_mrc.get_voting_manabar().current_mana ) - HIVE_VOTE_DUST_THRESHOLD );
    ...
    BOOST_REQUIRE( itr->get_rshares() == ( old_mana - _alice_mrc.get_voting_manabar().current_mana ) - HIVE_VOTE_DUST_THRESHOLD );
    validate_database();
    ...
    BOOST_REQUIRE( bob_comment_cashout->get_net_rshares() == ( old_manabar.current_mana - _alice_mrc.get_voting_manabar().current_mana ) - HIVE_VOTE_DUST_THRESHOLD );
    BOOST_REQUIRE( bob_comment_cashout->get_cashout_time() == bob_comment_cashout->get_creation_time() + HIVE_CASHOUT_WINDOW_SECONDS );
    BOOST_REQUIRE( itr != vote_idx.end() );
    validate_database();
    ...
    BOOST_REQUIRE( alice_comment_cashout->get_net_rshares() == old_net_rshares + ( old_mana - _bob_mrc.get_voting_manabar().current_mana ) - HIVE_VOTE_DUST_THRESHOLD );
    BOOST_REQUIRE( alice_comment_cashout->get_cashout_time() == alice_comment_cashout->get_creation_time() + HIVE_CASHOUT_WINDOW_SECONDS );
    BOOST_REQUIRE( itr != vote_idx.end() );
    validate_database();
    ...
    BOOST_REQUIRE( _alice_mrc.get_downvote_manabar().current_mana == 0 );
    BOOST_REQUIRE( _alice_mrc.get_voting_manabar().current_mana == old_manabar.current_mana - alice_weight + old_downvote_manabar.current_mana );
    validate_database();
...
BOOST_AUTO_TEST_CASE( vote_weights )
{
  try
  {
    ...
    for( i = 0; i < 40; ++i )
    {
      ...
      voterAssets.emplace_back( &db->get< assets_object, by_account_id >( voter.get_id() ) );
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
  try
  {
    ...
    const auto& new_bob_assets = db->get< assets_object, by_account_id >( new_bob.get_id() );
...
BOOST_AUTO_TEST_CASE(account_witness_vote_apply_delay)
{
  ...
  try
  {
    ...
    const auto& _alice = db->get_account("alice");
    const auto& _alice_assets = db->get< assets_object, by_account_id >( _alice.get_id() );
    const auto& _alice_dv = db->get< delayed_votes_object, by_account_id >( _alice.get_id() );
    const auto& _bob = db->get_account("bob");
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
  try
  {
    ...
    generate_blocks( 1 );
    const auto& alice_acc = db->get_account( "alice" );
    const auto& alice_mrc = db->get< manabars_rc_object, by_account_id >( alice_acc.get_id() );
    ...
    BOOST_REQUIRE( alice_comment_cashout->get_net_rshares() == old_manabar.current_mana - bob_mrc.get_voting_manabar().current_mana - HIVE_VOTE_DUST_THRESHOLD );
    BOOST_REQUIRE( itr->get_rshares() == old_manabar.current_mana - bob_mrc.get_voting_manabar().current_mana - HIVE_VOTE_DUST_THRESHOLD );
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      ...
      const auto& sam_time = db.get< time_object, by_account_id >( sam_acc.get_id() );
      db.modify( db.get< manabars_rc_object, by_account_id >( sam_acc.get_id() ), [&]( manabars_rc_object& mrc )
      {
