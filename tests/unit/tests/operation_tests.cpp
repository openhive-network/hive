...
BOOST_AUTO_TEST_CASE( vote_apply )
{
  try
  {
    ...
    const auto& _alice_time = _alice_assets;
    const auto& _sam_time = _sam_assets;
    const auto& _dave_time = _dave_assets;
    ...
    BOOST_REQUIRE( _alice_time.get_last_vote_time() == db->head_block_time() );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
  try
  {
    ...
    const auto& alice_assets = db->get< assets_object >( assets_object::id_type( alice.get_id().get_value() ) );
    const auto& alice_time = db->get< assets_object >( assets_object::id_type( alice.get_id().get_value() ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
  try
  {
    ...
    const auto& bob_assets = db->get< assets_object >( assets_object::id_type( bob_acc.get_id().get_value() ) );
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      ...
      db.modify( sam_assets, [&]( assets_object& a )
      {
        a.get_voting_manabar().current_mana = sam_acc.get_effective_vesting_shares( a ).value;
        a.get_voting_manabar().last_update_time = db.head_block_time().sec_since_epoch();
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      ...
      db.modify( sam_assets, [&]( assets_object& a )
      {
        a.get_downvote_manabar().current_mana = sam_acc.get_effective_vesting_shares( a ).value / 4;
        a.get_downvote_manabar().last_update_time = db.head_block_time().sec_since_epoch();
      });
    });
