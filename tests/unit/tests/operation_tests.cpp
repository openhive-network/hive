...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
  try
  {
    ...
    HIVE_REQUIRE_ASSERT( push_transaction( tx, { alice_private_key, bob_private_key } ),
      "account.get_vesting_withdraw_rate() != new_vesting_withdraw_rate || !account.has_active_power_down()" );
...
BOOST_AUTO_TEST_CASE(account_witness_vote_apply_delay)
{
  ...
  try
  {
    ...
    BOOST_REQUIRE(_alice_assets.get_vesting().amount == _alice.get_sum_delayed_votes().value);
