...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
  try
  {
    ...
    const account_authority_object& acct_auth = db->get< account_authority_object, by_name >( "alice" );
...
BOOST_AUTO_TEST_CASE( custom_binary_authorities )
{
  ...
  op.required_posting_auths.insert( "sam" );
  op.required_auths.push_back( db->get< account_authority_object, by_name >( "alice" ).posting );
  ...
  expected.push_back( db->get< account_authority_object, by_name >( "alice" ).posting );
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
  try
  {
    ...
    const auto& owner1 = db->get< account_authority_object, by_name >("bob").owner;
    ...
    const auto& owner2 = db->get< account_authority_object, by_name >("bob").owner;
    ...
    const auto& owner3 = db->get< account_authority_object, by_name >("bob").owner;
    ...
    const auto& owner4 = db->get< account_authority_object, by_name >("bob").owner;
    ...
    const auto& owner5 = db->get< account_authority_object, by_name >("bob").owner;
    ...
    const auto& owner6 = db->get< account_authority_object, by_name >("bob").owner;
    ...
    const auto& owner7 = db->get< account_authority_object, by_name >("bob").owner;
...
BOOST_AUTO_TEST_CASE( message_when_author_or_comment_doesnt_exist )
{
  try
  {
    ...
    HIVE_REQUIRE_EXCEPTION( db->get_comment( "unknown", string( "test" ) ), "!is_required && \"account\"", fc::exception );
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
  try
  {
    ...
    const auto& bob_meta = db->get< account_metadata_object, by_name >( bob.get_name() );
...
BOOST_AUTO_TEST_CASE( account_auth_tests )
{
  try
  {
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get< account_authority_object, by_name >( "alice"), [&]( account_authority_object& auth )
      {
        auth.active.add_authority( "bob", 1 );
        auth.posting.add_authority( "charlie", 1 );
      });

      db.modify( db.get< account_authority_object, by_name >( "bob" ), [&]( account_authority_object& auth )
      {
        auth.posting = authority( 1, bob_posting_private_key.get_public_key(), 1 );
      });

      db.modify( db.get< account_authority_object, by_name >( "charlie" ), [&]( account_authority_object& auth )
      {
...
BOOST_AUTO_TEST_CASE( account_update2_apply )
{
  try
  {
    ...
    const account_metadata_object& acct_metadata = db->get< account_metadata_object, by_name >( acct.get_name() );
