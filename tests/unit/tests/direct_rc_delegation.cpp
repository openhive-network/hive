#ifdef IS_TEST_NET
#include <boost/test/unit_test.hpp>

#include <hive/chain/detail/state/account_object.hpp>
#include <hive/chain/detail/state/comment_object.hpp>
#include <hive/chain/detail/state/witness_objects.hpp>
#include <hive/chain/detail/state/hardfork_property_object.hpp>
#include <hive/chain/detail/state/global_property_object.hpp>
// Multiindex headers for index type definitions
#include <hive/chain/detail/state/comment_object_multiindex.hpp>
#include <hive/chain/detail/state/transaction_object_multiindex.hpp>
#include <hive/chain/detail/state/dhf_objects_multiindex.hpp>
#include <hive/chain/detail/state/block_summary_object_multiindex.hpp>
#include <hive/chain/detail/state/hardfork_property_object_multiindex.hpp>
#include <hive/chain/detail/state/witness_objects_multiindex.hpp>
#include <hive/chain/detail/state/global_property_object_multiindex.hpp>
#include <hive/chain/detail/state/feed_history_object_multiindex.hpp>
#include <hive/chain/detail/state/limit_order_object_multiindex.hpp>
#include <hive/chain/detail/state/decline_voting_rights_request_object_multiindex.hpp>
#include <hive/chain/detail/state/reward_fund_object_multiindex.hpp>
#include <hive/chain/detail/state/recurrent_transfer_object_multiindex.hpp>
#include <hive/chain/detail/state/convert_request_object_multiindex.hpp>
#include <hive/chain/detail/state/collateralized_convert_request_object_multiindex.hpp>
#include <hive/chain/detail/state/escrow_object_multiindex.hpp>
#include <hive/chain/detail/state/savings_withdraw_object_multiindex.hpp>
#include <hive/chain/detail/state/liquidity_reward_balance_object_multiindex.hpp>
#include <hive/chain/detail/state/withdraw_vesting_route_object_multiindex.hpp>
#include <hive/chain/database_exceptions.hpp>
#include <hive/protocol/hive_operations.hpp>
#include <hive/protocol/hive_custom_operations.hpp>

#include <hive/chain/rc/rc_objects_multiindex.hpp>

#include "../db_fixture/clean_database_fixture.hpp"

#include <chrono>

using namespace hive::chain;
using namespace hive::protocol;

BOOST_FIXTURE_TEST_SUITE( direct_rc_delegation, clean_database_fixture )

BOOST_AUTO_TEST_CASE( delegate_rc_operation_validate )
{
  try{
    delegate_rc_operation op;
    op.from = "alice";
    op.delegatees = {"eve", "bob"};
    op.max_rc = 10;
    op.validate();

    // max_rc cannot be negative
    op.max_rc = -1;
    BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );
    op.max_rc = 10;

    // alice- is an invalid account name
    op.from = "alice-";
    BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

    // Bob- is an invalid account name
    op.from = "alice";
    op.delegatees = {"eve", "bob-"};
    BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

    // Alice is already the delegator
    op.delegatees = {"alice", "bob"};
    BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

    // Empty vector
    op.delegatees = {};
    BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

    // bob is defined twice, handled by the flat_set
    op.delegatees = {"eve", "bob", "bob"};
    op.validate();

    // There is more than HIVE_RC_MAX_ACCOUNTS_PER_DELEGATION_OP accounts in delegatees
    flat_set<account_name_type> set_too_big;
    for (int i = 0; i < HIVE_RC_MAX_ACCOUNTS_PER_DELEGATION_OP + 1; i++) {
      set_too_big.insert( "actor" + std::to_string(i) );
    }
    op.delegatees = set_too_big;
    BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( delegate_rc_operation_apply_single )
{
  try
  {
    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get_witness_schedule_object(), [&]( witness_schedule_object& wso )
      {
        wso.median_props.account_creation_fee = HIVE_asset( 3'000 ); //mainnet value
      } );
    } );
    generate_block();

    uint64_t min_delegation = 0;
    {
      const auto& gpo = db->get_dynamic_global_properties();
      const auto& wso = db->get_witness_schedule_object();
      min_delegation = ( HIVE_asset( wso.median_props.account_creation_fee.amount / 3 ) * gpo.get_vesting_share_price() ).get_amount();
    }

    BOOST_TEST_MESSAGE( "Testing:  delegate_rc_operation_apply_single to a single account" );
    ACTORS( DEFAULT_VESTING, (alice)(bob)(dave) );
    vest( "alice", HIVE_asset( 10'000 ) );
    int64_t alice_vests = get_vesting( "alice" ).amount.value;

    // Delegating more rc than I have should fail
    delegate_rc_operation op;
    op.from = "alice";
    op.delegatees = {"bob"};
    op.max_rc = alice_vests + 1;

    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Delegating less than the minimum should fail
    op.delegatees = {"bob"};
    op.max_rc = min_delegation - 1;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Delegating to a non-existing account should fail
    op.delegatees = {"eve"};
    op.max_rc = min_delegation;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Delegating 0 shouldn't work if there isn't already a delegation that exists (since 0 deletes the delegation)
    op.delegatees = {"bob"};
    op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Successful delegation
    op.delegatees = {"bob"};
    op.max_rc = min_delegation + 1;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, uint64_t(op.max_rc) );

    const account_object& from_rc_account = db->get_account( op.from );
    const account_object& to_rc_account = db->get_account( "bob" );

    BOOST_REQUIRE_EQUAL( from_rc_account.get_delegated_rc(), min_delegation + 1 );
    BOOST_REQUIRE_EQUAL( from_rc_account.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account.get_received_rc(), min_delegation + 1 );


    // Delegating the same amount shouldn't work
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Decrease the delegation
    op.from = "alice";
    op.delegatees = {"bob"};
    op.max_rc = min_delegation;
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* delegation_decreased = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation_decreased->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation_decreased->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation_decreased->delegated_rc, uint64_t(op.max_rc) );

    const account_object& from_rc_account_decreased = db->get_account( op.from );
    const account_object& to_rc_account_decreased = db->get_account( "bob" );

    BOOST_REQUIRE_EQUAL( from_rc_account_decreased.get_delegated_rc(), min_delegation );
    BOOST_REQUIRE_EQUAL( from_rc_account_decreased.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account_decreased.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account_decreased.get_received_rc(), min_delegation );

    // Increase the delegation
    op.from = "alice";
    op.delegatees = {"bob"};
    op.max_rc = min_delegation + 10;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* delegation_increased = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation_increased->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation_increased->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation_increased->delegated_rc, uint64_t(op.max_rc) );

    const account_object& from_rc_account_increased = db->get_account( op.from );
    const account_object& to_rc_account_increased = db->get_account( "bob" );

    BOOST_REQUIRE_EQUAL( from_rc_account_increased.get_delegated_rc(), min_delegation + 10 );
    BOOST_REQUIRE_EQUAL( from_rc_account_increased.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account_increased.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account_increased.get_received_rc(), min_delegation + 10 );

    // Delete the delegation
    op.from = "alice";
    op.delegatees = {"bob"};
    op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* delegation_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE( delegation_deleted == nullptr );

    const account_object& from_rc_account_deleted = db->get_account( "alice" );
    const account_object& to_rc_account_deleted = db->get_account( "bob" );

    BOOST_REQUIRE_EQUAL( from_rc_account_deleted.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( from_rc_account_deleted.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account_deleted.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account_deleted.get_received_rc(), 0 );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( delegate_rc_operation_apply_many )
{
  try
  {
    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get_witness_schedule_object(), [&]( witness_schedule_object& wso )
      {
        wso.median_props.account_creation_fee = HIVE_asset( 3'000 ); //mainnet value
      } );
    } );
    generate_block();

    uint64_t min_delegation = 0;
    {
      const auto& gpo = db->get_dynamic_global_properties();
      const auto& wso = db->get_witness_schedule_object();
      min_delegation = ( ( wso.median_props.account_creation_fee / 3 ) * gpo.get_vesting_share_price() ).get_amount();
    }

    BOOST_TEST_MESSAGE( "Testing:  delegate_rc_operation_apply_many to many accounts" );
    ACTORS( DEFAULT_VESTING, (alice)(bob)(dave)(dan) );
    vest( "alice", HIVE_asset( 10'000 ) );
    int64_t alice_vests = get_vesting( "alice" ).amount.value;

    // Delegating more rc than alice has should fail
    delegate_rc_operation op;
    op.from = "alice";
    op.delegatees = {"bob", "dave"};
    op.max_rc = alice_vests;

    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Delegating to a non-existing account should fail
    op.delegatees = {"bob", "eve"};
    op.max_rc = min_delegation;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Delegating 0 shouldn't work if there isn't already a delegation that exists (since 0 deletes the delegation)
    op.delegatees = {"dave", "bob"};
    op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Successful delegations
    op.delegatees = {"bob", "dave"};
    op.max_rc = min_delegation * 2;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, uint64_t(op.max_rc) );

    const account_object& from_rc_account = db->get_account( op.from );
    const account_object& to_rc_account = db->get_account( "bob" );

    BOOST_REQUIRE_EQUAL( from_rc_account.get_delegated_rc(), min_delegation * 4 );
    BOOST_REQUIRE_EQUAL( from_rc_account.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( to_rc_account.get_received_rc(), min_delegation * 2 );

    // Delegating the same amount shouldn't work
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Delegating 0 shouldn't work if there isn't already a delegation that exists (since 0 deletes the delegation)
    // dave/bob got a delegation but not dan so it should fail
    op.delegatees = {"dave", "bob", "dan"};
    op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    BOOST_CHECK_THROW( push_transaction(custom_op, alice_post_key), fc::exception );

    // Decrease the delegations
    op.from = "alice";
    op.delegatees = {"bob", "dave"};
    op.max_rc = min_delegation;
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);
    generate_block();

    const rc_direct_delegation_object* delegation_decreased_bob = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation_decreased_bob->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation_decreased_bob->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation_decreased_bob->delegated_rc, uint64_t(op.max_rc) );

    const rc_direct_delegation_object* delegation_decreased_dave = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
    BOOST_REQUIRE_EQUAL( delegation_decreased_dave->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation_decreased_dave->to, dave_id );
    BOOST_REQUIRE_EQUAL( delegation_decreased_dave->delegated_rc, uint64_t(op.max_rc) );

    const account_object& from_rc_account_decreased = db->get_account( op.from );
    const account_object& bob_rc_account_decreased = db->get_account( "bob" );
    const account_object& dave_rc_account_decreased = db->get_account( "dave" );

    BOOST_REQUIRE_EQUAL( from_rc_account_decreased.get_delegated_rc(), min_delegation * 2 );
    BOOST_REQUIRE_EQUAL( from_rc_account_decreased.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob_rc_account_decreased.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob_rc_account_decreased.get_received_rc(), min_delegation );
    BOOST_REQUIRE_EQUAL( dave_rc_account_decreased.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( dave_rc_account_decreased.get_received_rc(), min_delegation );

    // Increase and create a delegation
    op.from = "alice";
    op.delegatees = {"bob", "dan"};
    op.max_rc = min_delegation * 2;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* bob_delegation_increased = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    const rc_direct_delegation_object* dan_delegation_created = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dan_id ) );

    BOOST_REQUIRE_EQUAL( bob_delegation_increased->from, alice_id );
    BOOST_REQUIRE_EQUAL( bob_delegation_increased->to, bob_id );
    BOOST_REQUIRE_EQUAL( bob_delegation_increased->delegated_rc, uint64_t(op.max_rc) );
    BOOST_REQUIRE_EQUAL( dan_delegation_created->from, alice_id );
    BOOST_REQUIRE_EQUAL( dan_delegation_created->to, dan_id );
    BOOST_REQUIRE_EQUAL( dan_delegation_created->delegated_rc, uint64_t(op.max_rc) );

    const account_object& alice_rc_account_increased = db->get_account( op.from );
    const account_object& dan_rc_account_created = db->get_account( "dan" );
    const account_object& bob_rc_account_increased = db->get_account( "bob" );

    BOOST_REQUIRE_EQUAL( alice_rc_account_increased.get_delegated_rc(), min_delegation * 5 );
    BOOST_REQUIRE_EQUAL( alice_rc_account_increased.get_received_rc(), 0 );

    BOOST_REQUIRE_EQUAL( dan_rc_account_created.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( dan_rc_account_created.get_received_rc(), min_delegation * 2 );
    BOOST_REQUIRE_EQUAL( bob_rc_account_increased.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob_rc_account_increased.get_received_rc(), min_delegation * 2 );

    // Delete the delegations
    op.from = "alice";
    op.delegatees = {"bob", "dan", "dave"};
    op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction(custom_op, alice_post_key);

    generate_block();

    const rc_direct_delegation_object* delegation_bob_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    const rc_direct_delegation_object* delegation_dan_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dan_id ) );
    const rc_direct_delegation_object* delegation_dave_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dan_id ) );
    BOOST_REQUIRE( delegation_bob_deleted == nullptr );
    BOOST_REQUIRE( delegation_dan_deleted == nullptr );
    BOOST_REQUIRE( delegation_dave_deleted == nullptr );

    const account_object& from_rc_account_deleted = db->get_account( "alice" );
    const account_object& bob_rc_account_deleted = db->get_account( "bob" );
    const account_object& dave_rc_account_deleted = db->get_account( "dave" );
    const account_object& dan_rc_account_deleted = db->get_account( "dan" );

    BOOST_REQUIRE_EQUAL( from_rc_account_deleted.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( from_rc_account_deleted.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob_rc_account_deleted.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob_rc_account_deleted.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( dave_rc_account_deleted.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( dave_rc_account_deleted.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( dan_rc_account_deleted.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( dan_rc_account_deleted.get_received_rc(), 0 );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( delegate_rc_operation_apply_many_different )
{
  try
  {
    BOOST_TEST_MESSAGE( "Testing:  delegate_rc_operation_apply_many_different to many accounts" );
    generate_block();
    set_account_creation_fee( HIVE_asset( 300'000 ) ); //increased fee gives alice enough non-delegatable RC to
      //afford re-delegating without getting back the RC (only regen power) from the removed delegation to bob
    generate_block();
    ACTORS( DEFAULT_VESTING, (alice)(bob)(dave)(dan)(carol) );
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();
    vest( "alice", HIVE_asset( 10'000 ) );
    uint64_t alice_vests = get_vesting( "alice" ).amount.value;

    std::string json = "[";
    delegate_rc_operation op;
    op.from = "alice";
    op.delegatees = { "bob" };
    op.max_rc = alice_vests;
    json += "\"delegate_rc\",";
    json += fc::json::to_string( op );
    op.delegatees = { "dave" };
    op.max_rc = alice_vests - 1; //just to be different than that for bob
    json += ",\"delegate_rc\",";
    json += fc::json::to_string( op );
    op.delegatees = { "nonexistent" };
    op.max_rc = 0;
    json += ",\"delegate_rc\",";
    json += fc::json::to_string( op );
    json += "]";

    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = json;
    ilog( "${json}", (json) );

    // The following used to be true and is still true if operation is already in block:
    //   Delegating more rc than alice has should fail, but it does not, because of how interpreter
    //   works - only the first pair in outer array is processed when it is formatted this way
    //   see https://hiveblocks.com/tx/65b4772d915dfda66a254e5105c74b5c7667991a
    //   it also means that even "delegating" to nonexistent account will pass since it is ignored
    // Now interpreter is more strict when node is_in_control()
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, alice_post_key ), "!db.is_in_control() && \"Expected pair of values: [ operation_name_or_id, operation ]\"" );
    // Let's put just the correct part into transaction so we don't have to change rest of the test
    json = "[";
    op.delegatees = { "bob" };
    op.max_rc = alice_vests;
    json += "\"delegate_rc\",";
    json += fc::json::to_string( op );
    json += "]";

    custom_op.json = json;
    ilog( "${json}", (json) );

    push_transaction( custom_op, alice_post_key );

    generate_block();

    const rc_direct_delegation_object* delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, alice_vests );
    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
    BOOST_REQUIRE( delegation == nullptr ); // second part of ill-formed json was ignored so there is no delegation

    op.delegatees = { "bob" };
    op.max_rc = 0;
    json = "[\"delegate_rc\"," + fc::json::to_string( op ) + "]";
    custom_op.json = json; // use proper legacy format to remove delegation
    ilog( "${json}", (json) );

    push_transaction( custom_op, alice_post_key );

    generate_blocks( db->head_block_time() + fc::seconds( HIVE_RC_REGEN_TIME ) ); //regenerate mana
    generate_block();

    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE( delegation == nullptr ); // delegation should be removed properly

    op.max_rc = alice_vests / 3;
    std::string delegate_rc_type = fc::json::to_string( hive::protocol::rc_custom_operation( op ).which() );
    json = "[" + delegate_rc_type + "," + fc::json::to_string( op ) + "]";
    custom_op.json = json; // use alternative (even smaller) legacy format to add delegation
    ilog( "${json}", (json) );

    push_transaction( custom_op, alice_post_key );

    generate_block();
    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, bob_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, alice_vests / 3 );

    // now delegate to three accounts using the most compact form (alternative legacy in array)
    // (actually the extension that is constantly added could be dropped since empty is default)
    json = "[";
    op.delegatees = { "bob" };
    op.max_rc = 0;
    json += "[" + delegate_rc_type + "," + fc::json::to_string(op) + "]";
    op.delegatees = { "dave" };
    op.max_rc = alice_vests / 4;
    json += ",[" + delegate_rc_type + "," + fc::json::to_string( op ) + "]";
    op.delegatees = { "dan", "carol" };
    op.max_rc = alice_vests / 5;
    json += ",[" + delegate_rc_type + "," + fc::json::to_string( op ) + "]";
    json += "]";
    // double array is important because it tells the interpreter to think of it as array
    // and not as tuple; note that each part is in fact separate operation (executed in order),
    // therefore it would not be ok if alice could not afford delegation without undelegating of bob
    // since she only gets back regen power, not the RC itself (operations don't balance out when
    // they are not part of single delegation)
    custom_op.json = json;
    ilog( "${json}", (json) );

    push_transaction( custom_op, alice_post_key );

    generate_block();
    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE( delegation == nullptr );
    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, dave_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, alice_vests / 4 );
    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dan_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, dan_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, alice_vests / 5 );
    delegation = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, carol_id ) );
    BOOST_REQUIRE_EQUAL( delegation->from, alice_id );
    BOOST_REQUIRE_EQUAL( delegation->to, carol_id );
    BOOST_REQUIRE_EQUAL( delegation->delegated_rc, alice_vests / 5 );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( update_outdel_overflow )
{
  try
  {
    BOOST_TEST_MESSAGE( "Testing:  update_outdel_overflow" );
    generate_block();
    set_account_creation_fee( HIVE_asset( 3'000 ) ); //realistic fee gives accounts a sizable non-delegatable RC adjustment
    generate_block();

    ACTORS( NO_VESTING, (alice)(bob)(dave)(eve)(martin) );
    generate_block();
    const account_object& alice = db->get_account( "alice" );
    const account_object& bob = db->get_account( "bob" );
    const account_object& dave = db->get_account( "dave" );
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();
    vest( "alice", HIVE_asset( 10 ) );
    generate_block();

    int64_t vesting_amount = alice.get_vesting().amount.value;
    int64_t creation_rc = alice.get_rc_adjustment().value;
    int64_t delegate_rc_cost = 0;

    // Delegate 10 rc to bob, the rest to dave, alice has rc_adjustment remaining rc
    delegate_rc_operation op;
    op.from = "alice";
    op.delegatees = { "bob" };
    op.max_rc = 10;
    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    delegate_rc_cost += push_transaction( custom_op, alice_post_key )->get_rc_cost();
    op.delegatees = { "dave" };
    op.max_rc = vesting_amount - 10 + 1;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    //check that it is not possible to dip into creation_rc, nor will it be accepted at the cost of dropping bob
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, alice_post_key ), "from_delegable_rc >= delta_total" );

    op.max_rc = vesting_amount - 10;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    delegate_rc_cost += push_transaction( custom_op, alice_post_key )->get_rc_cost();
    generate_block();

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t(vesting_amount) );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( alice.rc_manabar.current_mana, creation_rc - delegate_rc_cost ); // cost of the two delegate rc ops

    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 10 );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + vesting_amount - 10 );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + vesting_amount - 10 );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t( vesting_amount - 10 ) );

    // regenerate RC mana fully
    generate_blocks( db->head_block_time() + HIVE_RC_REGEN_TIME );
    delegate_rc_cost = 0;

    // we delegate and it affects one delegation
    // Delegate 5 vests out, alice has 5 remaining rc, it's lower than the rc_adjustment which is 10
    // So the first delegation (to bob) is lowered to 5
    delegate_vesting_shares_operation dvso;
    dvso.vesting_shares = ASSET( "0.000005 VESTS" );
    dvso.delegator = "alice";
    dvso.delegatee = { "eve" };
    delegate_rc_cost += push_transaction( dvso, alice_private_key )->get_rc_cost();

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t(vesting_amount) - 5 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( alice.rc_manabar.current_mana, creation_rc - delegate_rc_cost );

    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, creation_rc + 5 );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc + 5 );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 5 );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + vesting_amount - 10 );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + vesting_amount - 10 );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t( vesting_amount - 10 ) );

    // We delegate and we don't have enough rc to sustain bob's delegation
    dvso.vesting_shares = ASSET( "0.000006 VESTS" );
    dvso.delegator = "alice";
    dvso.delegatee = "martin";
    delegate_rc_cost += push_transaction( dvso, alice_private_key )->get_rc_cost();

    const rc_direct_delegation_object* delegation_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
    BOOST_REQUIRE( delegation_deleted == nullptr );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t(vesting_amount) - 11 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( alice.rc_manabar.current_mana, creation_rc - delegate_rc_cost );

    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 0 );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + vesting_amount - 11 );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + vesting_amount - 11 );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t( vesting_amount - 11 ) );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( update_outdel_overflow_many_accounts )
{
  try
  {
    BOOST_TEST_MESSAGE( "Testing:  update_outdel_overflow with many actors" );
    generate_block();
    const int removal_limit = 200; //current mainnet value - if we increase it (we should, but it requires HF) we need to change it here as well
    set_account_creation_fee( HIVE_asset( 3'000 ) ); //realistic fee gives accounts a sizable non-delegatable RC adjustment
    db_plugin->debug_update( [=]( database& db )
    {
      db.set_remove_threshold( removal_limit );
    } );
    generate_block();
    #define NUM_ACTORS 250
    for( int i = 0; i < NUM_ACTORS; ++i )
      account_create( "actor" + std::to_string(i), init_account_pub_key, init_account_pub_key, NO_VESTING );
    ACTORS( NO_VESTING, (alice)(bob) );
    generate_block();
    const account_object& alice = db->get_account( "alice" );
    const account_object& actor0 = db->get_account( "actor0" );
    const account_object& actor2 = db->get_account( "actor2" );
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();

    vest( "alice", HIVE_asset( 10'000 ) );
    generate_block();

    uint64_t vesting_amount = uint64_t( alice.get_vesting().amount.value );
    int64_t creation_rc = alice.get_rc_adjustment().value;

    delegate_rc_operation op;
    op.from = "alice";
    op.max_rc = 10;
    // Delegate 10 rc to every actor account
    int count = 0;
    for( int i = 0; i < NUM_ACTORS; ++i )
    {
      op.delegatees.insert( "actor" + std::to_string(i) );
      if( count == 50 || i == NUM_ACTORS -1 )
      {
        push_custom_json_operation( {}, { "alice" }, HIVE_RC_CUSTOM_OPERATION_ID, fc::json::to_string( hive::protocol::rc_custom_operation( op ) ), alice_post_key );
        generate_block();
        op.delegatees = {};
        count = 0;
      }
      ++count;
    }

    // regenerate RC mana fully after setting up delegations
    generate_blocks( db->head_block_time() + HIVE_RC_REGEN_TIME );

    // We delegate to bob last so that his delegation would be the last to be affected
    op.delegatees = { "bob" };
    op.max_rc = vesting_amount - NUM_ACTORS * 10 ;
    push_custom_json_operation( {}, { "alice" }, HIVE_RC_CUSTOM_OPERATION_ID, fc::json::to_string( hive::protocol::rc_custom_operation( op ) ), alice_post_key );
    generate_block();

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), vesting_amount );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    BOOST_REQUIRE_EQUAL( actor0.rc_manabar.current_mana, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( actor0.last_max_rc, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( actor0.get_received_rc(), 10 );

    BOOST_REQUIRE_EQUAL( actor2.rc_manabar.current_mana, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( actor2.last_max_rc, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( actor2.get_received_rc(), 10 );

    // we delegate 25 vests and it affects the first three delegations
    // Delegate 25 vests out, alice has rc deficit (she can't dip to creation_rc that results from rc_adjustment)
    // So the first two delegations (to actor0 and actor1) are deleted while the delegation to actor2 is halved
    delegate_vesting_shares_operation dvso;
    dvso.vesting_shares = ASSET( "0.000025 VESTS" );
    dvso.delegator = "alice";
    dvso.delegatee = "bob";
    push_transaction( dvso, alice_private_key );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), vesting_amount - 25 ); // total amount minus what was delegated
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    BOOST_REQUIRE_EQUAL( actor0.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( actor0.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( actor0.get_received_rc(), 0 );

    BOOST_REQUIRE_EQUAL( actor2.rc_manabar.current_mana, creation_rc + 5 );
    BOOST_REQUIRE_EQUAL( actor2.last_max_rc, creation_rc + 5 );
    BOOST_REQUIRE_EQUAL( actor2.get_received_rc(), 5 );

    {
      const rc_direct_delegation_object* delegation_actor0_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, get_account_id( "actor0" ) ) );
      BOOST_REQUIRE( delegation_actor0_deleted == nullptr );
      const rc_direct_delegation_object* delegation_actor1_deleted = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, get_account_id( "actor1" ) ) );
      BOOST_REQUIRE( delegation_actor1_deleted == nullptr );
    }

    // We check that the rest of the delegations weren't affected
    for( int i = 4; i < NUM_ACTORS; ++i )
    {
      const account_object& actor = db->get_account( "actor" + std::to_string(i) );
      BOOST_REQUIRE_EQUAL( actor.rc_manabar.current_mana, creation_rc + 10 );
      BOOST_REQUIRE_EQUAL( actor.last_max_rc, creation_rc + 10 );
      BOOST_REQUIRE_EQUAL( actor.get_received_rc(), 10 );
    }

    // We delegate all our vests and we don't have enough to sustain any of our remaining delegations
    dvso.vesting_shares = alice.get_vesting().to_asset();
    dvso.delegator = "alice";
    dvso.delegatee = "bob";
    push_transaction( dvso, alice_private_key );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    // we might wait couple of blocks due to limit on delegation removal
    int i = NUM_ACTORS - removal_limit;
    while( i > 0 )
    {
      generate_block();
      i -= removal_limit;
    }
    // We check that every delegation got deleted
    for( int i = 0; i < NUM_ACTORS; ++i )
    {
      const account_object& actor = db->get_account( "actor" + std::to_string(i) );
      BOOST_REQUIRE_EQUAL( actor.rc_manabar.current_mana, creation_rc );
      BOOST_REQUIRE_EQUAL( actor.last_max_rc, creation_rc );
      BOOST_REQUIRE_EQUAL( actor.get_received_rc(), 0 );
    }

    // Remove vests delegation and check that we don't get the rc back immediately
    dvso.vesting_shares = ASSET( "0.000000 VESTS" );
    dvso.delegator = "alice";
    dvso.delegatee = "bob";
    push_transaction( dvso, alice_private_key );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( direct_rc_delegation_vesting_withdrawal )
{
  try
  {
    generate_block();
    set_account_creation_fee( HIVE_asset( 300'000 ) ); //set increased fee so alice can regenerate fully after transaction on RC adjustment
      //alone in the shortened time of single withdrawal step (it is an hour instead of week due to configuration settings)
    generate_block();
    ACTORS( NO_VESTING, (alice)(bob)(dave) );
    generate_block();
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();
    const account_object& alice = db->get_account( "alice" );
    const account_object& bob = db->get_account( "bob" );
    const account_object& dave = db->get_account( "dave" );

    vest( "alice", HIVE_asset( 10'000 ) );
    generate_block();

    int64_t creation_rc = alice.get_rc_adjustment().value;
    BOOST_REQUIRE_EQUAL( creation_rc, bob.get_rc_adjustment().value );
    BOOST_REQUIRE_EQUAL( creation_rc, dave.get_rc_adjustment().value );
    int64_t vesting_shares = alice.get_vesting().amount.value;
    int64_t delegated_rc = vesting_shares / 2;
    // 2*(vesting_shares/2) leaves 1 undelegated 'spare' vest in case vesting_shares is odd
    int64_t spare = vesting_shares - 2 * delegated_rc;

    BOOST_TEST_MESSAGE( "Setting up rc delegations" );

    delegate_rc_operation drc_op;
    drc_op.from = "alice";
    drc_op.delegatees = { "bob", "dave" };
    drc_op.max_rc = delegated_rc;
    push_custom_json_operation( {}, { "alice" }, HIVE_RC_CUSTOM_OPERATION_ID, fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) ), alice_post_key );
    generate_block();

    BOOST_TEST_MESSAGE( "Setting up withdrawal" );

    withdraw_vesting_operation op;
    op.account = "alice";
    op.vesting_shares = alice.get_vesting().to_asset();
    push_transaction( op, alice_private_key );

    auto next_withdrawal = db->head_block_time() + HIVE_VESTING_WITHDRAW_INTERVAL_SECONDS;
    VEST_asset withdraw_rate = alice.vesting_withdraw_rate;

    BOOST_TEST_MESSAGE( "Generating block up to first withdrawal" );
    generate_blocks( next_withdrawal - HIVE_BLOCK_INTERVAL );

    BOOST_REQUIRE_EQUAL( get_vesting( "alice" ).amount.value, vesting_shares );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t( vesting_shares - withdraw_rate.amount.value ) ); // total amount minus what was withdrew
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    // There wasn't enough to sustain the delegation to bob, so it got modified
    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, delegated_rc + creation_rc - withdraw_rate.amount.value + spare );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), uint64_t( delegated_rc - withdraw_rate.amount.value + spare ) );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, delegated_rc - withdraw_rate.amount.value + creation_rc + spare );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + delegated_rc );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + delegated_rc );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t(delegated_rc) );

    {
      const rc_direct_delegation_object* delegation_bob = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
      BOOST_REQUIRE_EQUAL( delegation_bob->from, alice_id );
      BOOST_REQUIRE_EQUAL( delegation_bob->to, bob_id );
      BOOST_REQUIRE_EQUAL( delegation_bob->delegated_rc, uint64_t( delegated_rc - withdraw_rate.amount.value + spare ) );
      const rc_direct_delegation_object* delegation_dave = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
      BOOST_REQUIRE_EQUAL( delegation_dave->from, alice_id );
      BOOST_REQUIRE_EQUAL( delegation_dave->to, dave_id );
      BOOST_REQUIRE_EQUAL( delegation_dave->delegated_rc, uint64_t(delegated_rc) );
    }

    BOOST_TEST_MESSAGE( "Generating block to cause withdrawal" );
    generate_block();

    for( int i = 1; i < HIVE_VESTING_WITHDRAW_INTERVALS - 1; ++i )
    {
      ilog( "Withdrawal step ${i}", (i) );
      // generate up to just before the withdrawal
      generate_blocks( db->head_block_time() + HIVE_VESTING_WITHDRAW_INTERVAL_SECONDS - HIVE_BLOCK_INTERVAL);

      const rc_direct_delegation_object* delegation_bob = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
      const rc_direct_delegation_object* delegation_dave = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
      auto total_vests_withdrew = withdraw_rate.amount.value * (i + 1);

      BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t( vesting_shares - total_vests_withdrew ) ); // total amount minus what was withdrew
      BOOST_REQUIRE_EQUAL( alice.rc_manabar.current_mana, creation_rc );
      BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
      BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

      if( total_vests_withdrew < delegated_rc + spare )
      {
        // Only the first delegation is affected
        BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, delegated_rc + creation_rc - total_vests_withdrew + spare );
        BOOST_REQUIRE_EQUAL( bob.get_received_rc(), uint64_t( delegated_rc - total_vests_withdrew + spare ) );
        BOOST_REQUIRE_EQUAL( bob.last_max_rc, delegated_rc + creation_rc - total_vests_withdrew + spare );

        BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, delegated_rc + creation_rc );
        BOOST_REQUIRE_EQUAL( dave.last_max_rc, delegated_rc + creation_rc );
        BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t(delegated_rc) );

        BOOST_REQUIRE_EQUAL( delegation_bob->from, alice_id );
        BOOST_REQUIRE_EQUAL( delegation_bob->to, bob_id );
        BOOST_REQUIRE_EQUAL( delegation_bob->delegated_rc, uint64_t( delegated_rc - total_vests_withdrew + spare ) );

        BOOST_REQUIRE_EQUAL( delegation_dave->from, alice_id );
        BOOST_REQUIRE_EQUAL( delegation_dave->to, dave_id );
        BOOST_REQUIRE_EQUAL( delegation_dave->delegated_rc, uint64_t(delegated_rc) );
      }
      else
      {
        // the first delegation is deleted and the second delegation starts to be affected
        BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, creation_rc );
        BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 0 );
        BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc );
        BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + delegated_rc - ( total_vests_withdrew - delegated_rc ) + spare );
        BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t(delegated_rc - ( total_vests_withdrew - delegated_rc ) + spare) );
        BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + delegated_rc - ( total_vests_withdrew - delegated_rc ) + spare );

        BOOST_REQUIRE( delegation_bob == nullptr );

        BOOST_REQUIRE_EQUAL( delegation_dave->from, alice_id );
        BOOST_REQUIRE_EQUAL( delegation_dave->to, dave_id );
        BOOST_REQUIRE_EQUAL( delegation_dave->delegated_rc, uint64_t( vesting_shares - total_vests_withdrew ) );
      }

      generate_block();
    }

    BOOST_TEST_MESSAGE( "Generating one more block to finish vesting withdrawal" );
    generate_blocks( db->head_block_time() + HIVE_VESTING_WITHDRAW_INTERVAL_SECONDS, true );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 0 );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), 0 );

    {
      const rc_direct_delegation_object* delegation_bob = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
      BOOST_REQUIRE( delegation_bob == nullptr );
      const rc_direct_delegation_object* delegation_dave = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
      BOOST_REQUIRE( delegation_dave == nullptr );
    }
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( direct_rc_delegation_vesting_withdrawal_routes )
{
  try
  {
    generate_block();
    set_account_creation_fee( HIVE_asset( 300'000 ) ); //set increased fee so alice can regenerate fully after transaction on RC adjustment
      //alone in the shortened time of single withdrawal step (it is an hour instead of week due to configuration settings)
    generate_block();
    ACTORS( NO_VESTING, (alice)(bob)(dave) );
    generate_block();
    const account_object& alice = db->get_account( "alice" );
    const account_object& bob = db->get_account( "bob" );
    const account_object& dave = db->get_account( "dave" );
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();

    vest( "alice", HIVE_asset( 10'000 ) );
    generate_block();

    int64_t creation_rc = alice.get_rc_adjustment().value;
    int64_t vesting_shares = alice.get_vesting().amount.value;
    int64_t delegated_rc = vesting_shares / 2;

    BOOST_TEST_MESSAGE( "Setting up vesting routes" );
    set_withdraw_vesting_route( "alice", "bob", HIVE_1_PERCENT * 25, true, alice_private_key );
    set_withdraw_vesting_route( "alice", "alice", HIVE_1_PERCENT * 50, true, alice_private_key );
    set_withdraw_vesting_route( "alice", "dave", HIVE_1_PERCENT * 25, false, alice_private_key );

    // regenerate RC mana fully after setting up vesting routes
    generate_blocks( db->head_block_time() + HIVE_RC_REGEN_TIME );

    BOOST_TEST_MESSAGE( "Setting up rc delegations" );

    delegate_rc_operation drc_op;
    drc_op.from = "alice";
    drc_op.delegatees = { "bob", "dave" };
    drc_op.max_rc = delegated_rc;
    push_custom_json_operation( {}, { "alice" }, HIVE_RC_CUSTOM_OPERATION_ID, fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) ), alice_post_key );
    generate_block();

    BOOST_TEST_MESSAGE( "Setting up withdrawal" );

    withdraw_vesting_operation op;
    op.account = "alice";
    op.vesting_shares = alice.get_vesting().to_asset();
    push_transaction( op, alice_private_key );

    auto next_withdrawal = db->head_block_time() + HIVE_VESTING_WITHDRAW_INTERVAL_SECONDS;
    VEST_asset withdraw_rate = alice.vesting_withdraw_rate;

    BOOST_TEST_MESSAGE( "Generating block up to first withdrawal" );
    generate_blocks( next_withdrawal - HIVE_BLOCK_INTERVAL );

    BOOST_REQUIRE_EQUAL( get_vesting( "alice" ).amount.value, vesting_shares );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t( vesting_shares - withdraw_rate.amount.value ) ); // total amount minus what was withdrawn
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    // There wasn't enough to sustain the delegation to bob, so it got modified
    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, delegated_rc + creation_rc - withdraw_rate.amount.value );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), uint64_t( delegated_rc - withdraw_rate.amount.value ) );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, delegated_rc - withdraw_rate.amount.value + creation_rc );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + delegated_rc );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + delegated_rc );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t(delegated_rc) );

    {
      const rc_direct_delegation_object* delegation_bob = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
      BOOST_REQUIRE_EQUAL( delegation_bob->from, alice_id );
      BOOST_REQUIRE_EQUAL( delegation_bob->to, bob_id );
      BOOST_REQUIRE_EQUAL( delegation_bob->delegated_rc, uint64_t( delegated_rc - withdraw_rate.amount.value ) );
      const rc_direct_delegation_object* delegation_dave = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
      BOOST_REQUIRE_EQUAL( delegation_dave->from, alice_id );
      BOOST_REQUIRE_EQUAL( delegation_dave->to, dave_id );
      BOOST_REQUIRE_EQUAL( delegation_dave->delegated_rc, uint64_t(delegated_rc) );
    }

    BOOST_TEST_MESSAGE( "Generating block to cause withdrawal" );
    generate_block();

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), uint64_t( vesting_shares - ( withdraw_rate.amount.value * 2 - withdraw_rate.amount.value / 2 ) ) );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.rc_manabar.current_mana, creation_rc ); // withdraw_rate.amount.value is withdrawn, but half is auto vested back
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    // There wasn't enough to sustain the delegation to bob, so it got modified
    // We remove *1.5 instead of *2 because alice auto vested half, so she only needed to remove 0.5 of the withdraw_rate to reach creation_rc this round
    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, delegated_rc + creation_rc - withdraw_rate.amount.value * 1.5 + withdraw_rate.amount.value / 4 );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), uint64_t( delegated_rc - withdraw_rate.amount.value * 1.5 ) );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, delegated_rc + creation_rc - withdraw_rate.amount.value * 1.5  + withdraw_rate.amount.value / 4 );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + delegated_rc );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + delegated_rc );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), uint64_t(delegated_rc) );

    {
      const rc_direct_delegation_object* delegation_bob = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, bob_id ) );
      BOOST_REQUIRE_EQUAL( delegation_bob->from, alice_id );
      BOOST_REQUIRE_EQUAL( delegation_bob->to, bob_id );
      BOOST_REQUIRE_EQUAL( delegation_bob->delegated_rc, uint64_t( delegated_rc - withdraw_rate.amount.value * 1.5 ) );
      const rc_direct_delegation_object* delegation_dave = db->find< rc_direct_delegation_object, by_from_to >( boost::make_tuple( alice_id, dave_id ) );
      BOOST_REQUIRE_EQUAL( delegation_dave->from, alice_id );
      BOOST_REQUIRE_EQUAL( delegation_dave->to, dave_id );
      BOOST_REQUIRE_EQUAL( delegation_dave->delegated_rc, uint64_t(delegated_rc) );
    }

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( rc_delegation_regeneration )
{
  try
  {
    generate_block();
    set_account_creation_fee( HIVE_asset( 3'000 ) ); // realistic fee gives bob enough RC adjustment for him to have
      //enough mana to burn in the test below (realistic vest price makes minimal fee give almost nothing)
    generate_block();

    ACTORS( NO_VESTING, (alice)(bob) );
    generate_block();
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();

    fund( "bob", HBD_asset( 100'000 ) );
    vest( "alice", HIVE_asset( 1'000 ) );
    generate_block();

    BOOST_TEST_MESSAGE( "Fetching the baseLine with no RC delegations" );

    auto burn_mana = [this]( const account_name_type& account_name, int64_t amount_to_burn, const fc::ecc::private_key& key )
    {
      const account_object& account = db->get_account( account_name );
      auto initial_mana = account.rc_manabar.current_mana;
      auto current_mana = initial_mana;

      // burn mana with self-transfers (use memo to avoid duplicates)
      while( current_mana + amount_to_burn > initial_mana )
      {
        transfer( account_name, account_name, ASSET( "1.000 TBD" ), std::to_string( current_mana ), key );
        current_mana = account.rc_manabar.current_mana;
      }

      return current_mana;
    };
    auto mana_regen_per_second = [this]( const account_name_type& account_name, int64_t current_mana )
    {
      const account_object& account = db->get_account( account_name );
      hive::chain::util::manabar_params manabar_params( account.get_maximum_rc().value, HIVE_RC_REGEN_TIME );
      auto manabar = account.rc_manabar;
      // Magic number: we regenerate based off the future, because otherwise the manabar will already be up to date and won't regenerate
      manabar.regenerate_mana( manabar_params, db->get_dynamic_global_properties().get_head_block_time().sec_since_epoch() + 1 );
      return manabar.current_mana - current_mana;
    };

    int64_t current_mana = burn_mana( "bob", 50000000, bob_private_key );
    int64_t delta_no_delegation = mana_regen_per_second( "bob", current_mana );
    ilog( "Regeneration before delegation: ${d}", ( "d", delta_no_delegation ) );

    BOOST_TEST_MESSAGE( "Delegating all the RC" );
    delegate_rc_operation drc_op;
    drc_op.from = "alice";
    drc_op.delegatees = { "bob" };
    drc_op.max_rc = db->get_account( "alice" ).get_vesting().amount.value;
    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) );
    push_transaction( custom_op, alice_post_key );
    generate_block();

    current_mana = burn_mana( "bob", 50000000, bob_private_key );
    int64_t delta_delegation = mana_regen_per_second( "bob", current_mana );
    ilog( "Regeneration with full delegation: ${d}", ( "d", delta_delegation ) );

    BOOST_REQUIRE_GT( delta_delegation, delta_no_delegation );

    BOOST_TEST_MESSAGE( "Reducing the RC delegation" );
    drc_op.max_rc = db->get_account( "alice" ).get_vesting().amount.value / 2;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) );
    push_transaction( custom_op, alice_post_key );
    generate_block();

    current_mana = burn_mana( "bob", 50000000, bob_private_key );
    int64_t delta_reduced_delegation = mana_regen_per_second( "bob", current_mana );
    ilog( "Regeneration with smaller delegation: ${d}", ( "d", delta_reduced_delegation ) );

    BOOST_TEST_MESSAGE( "Removing the RC delegation" );
    drc_op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) );
    push_transaction( custom_op, alice_post_key );
    generate_block();

    BOOST_REQUIRE_GT( delta_delegation, delta_reduced_delegation );

    current_mana = burn_mana( "bob", 50000000, bob_private_key );
    int64_t delta_removed_delegation = mana_regen_per_second( "bob", current_mana );
    ilog( "Regeneration with removed delegation: ${d}", ( "d", delta_removed_delegation ) );

    BOOST_REQUIRE_GT( delta_delegation, delta_removed_delegation );
    BOOST_REQUIRE_GT( delta_reduced_delegation, delta_removed_delegation );
    BOOST_REQUIRE_EQUAL( delta_removed_delegation, delta_no_delegation );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( rc_delegation_removal_no_rc )
{
  try
  {
    generate_block();
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();

    ACTORS( NO_VESTING, (alice)(bob) );
    generate_block();
    const account_object& bob = db->get_account( "bob" );
    ilog( "bob's rc_adjustment = ${a}", ( "a", bob.get_rc_adjustment() ) );
    fund( "bob", HIVE_asset( 9'000'000'000 ) );
    vest( "alice", HIVE_asset( 10'000 ) );
    generate_block();

    BOOST_TEST_MESSAGE( "delegating RC to bob" );
    delegate_rc_operation drc_op;
    drc_op.from = "alice";
    drc_op.delegatees = {"bob"};
    drc_op.max_rc = 100'000'000;
    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "alice" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) );
    push_transaction( custom_op, alice_post_key );
    generate_block();

    const auto& current_mana = bob.rc_manabar.current_mana;

    // Magic number (approx. cost of single tx), it's hard to exactly hit 0 RC, but removing the delegation should put us below base rc_adjustment
    while( current_mana > 28'000'000 )
    {
      ilog( "bob's RC mana = ${m}", ( "m", current_mana ) );
      auto mana_before = current_mana;
      transfer_to_vesting_operation op;
      op.from = "bob";
      op.to = "alice";
      op.amount = ASSET( "0.001 TESTS" );
      push_transaction( op, bob_private_key );
      generate_block();
      ilog( "transaction's RC cost = ${c}", ( "c", mana_before - current_mana ) );
    }

    ilog( "bob's RC mana before undelegate = ${m}", ( "m", current_mana ) );
    BOOST_TEST_MESSAGE( "Removing the RC delegation" );
    drc_op.max_rc = 0;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( drc_op ) );
    push_transaction( custom_op, alice_post_key );
    ilog( "bob's RC mana after undelegate = ${m}", ( "m", current_mana ) );
    generate_block();

    BOOST_REQUIRE_EQUAL( current_mana, 0 );
    BOOST_REQUIRE_EQUAL( bob.get_maximum_rc(), bob.get_rc_adjustment() );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( rc_negative_regeneration_bug )
{
  try
  {
    set_account_creation_fee( HIVE_asset( 3'000 ) ); // realistic fee gives accounts necessary base RC mana
    generate_block();

    BOOST_TEST_MESSAGE( "Negative RC regeneration bug" );
    auto auto_reset( set_mainnet_cashout_values() ); //time is needed for RC to regenerate - testnet values are too small

    //the problem is very cryptic
    //first thing - it involves negative RC which shouldn't really ever happen (but does, for this
    //test it will be just set artificially)
    //second - it needs to be receiver of RC delegation that loses that delegation due to delegator
    //powering down more than he left undelegated
    //third - the bug only shows when significant time passed between the moment when RC of delegatee
    //went negative and the moment it lost delegation - the bug shows as delegatee having less RC
    //than he would have if RC regeneration was triggered by other means

    ACTORS( DEFAULT_VESTING, (delegator1)(delegator2)(delegator3)(delegatee)(pattern2)(pattern3) );
    generate_block();
    vest( "delegator1", HIVE_asset( 1'000'000 ) );
    vest( "delegator2", HIVE_asset( 1'000'000 ) );
    vest( "delegator3", HIVE_asset( 1'000'000 ) );
    auto full_vest = get_vesting( "delegator1" );
    BOOST_REQUIRE_EQUAL( full_vest, get_vesting( "delegator2" ) );
    BOOST_REQUIRE_EQUAL( full_vest, get_vesting( "delegator3" ) );

    const auto& delegatee_rc = db->get_account( "delegatee" );
    const auto& pattern2_rc = db->get_account( "pattern2" );
    const auto& pattern3_rc = db->get_account( "pattern3" );
    BOOST_REQUIRE_EQUAL( delegatee_rc.rc_manabar.current_mana, pattern2_rc.rc_manabar.current_mana );
    BOOST_REQUIRE_EQUAL( delegatee_rc.rc_manabar.current_mana, pattern3_rc.rc_manabar.current_mana );

    BOOST_TEST_MESSAGE( "Pattern2 makes a comment and delegator2 votes on it - payout will trigger RC regen" );
    comment_operation comment;
    comment.parent_permlink = "test";
    comment.author = "pattern2";
    comment.permlink = "test";
    comment.title = "test";
    comment.body = "test";
    push_transaction( comment, pattern2_post_key );
    generate_block();
    auto cashout_time = db->find_comment_cashout( *db->get_comment( "pattern2", std::string( "test" ) ) )->get_cashout_time();

    vote_operation vote;
    vote.voter = "delegator2";
    vote.author = "pattern2";
    vote.permlink = "test";
    vote.weight = HIVE_100_PERCENT;
    push_transaction( vote, delegator2_post_key );
    generate_block();

    //wait so the RC used for comment is restored
    generate_blocks( db->head_block_time() + HIVE_RC_REGEN_TIME );

    BOOST_TEST_MESSAGE( "All delegators make RC delegations to their delegatees with the same power" );
    auto rc_delegate = [&]( const account_name_type& from, const account_name_type& to, const VEST_asset& amount, const fc::ecc::private_key& post_key )
    {
      delegate_rc_operation delegate;
      delegate.from = from;
      delegate.delegatees = { to };
      delegate.max_rc = amount.amount.value;
      push_custom_json_operation( {}, { from }, HIVE_RC_CUSTOM_OPERATION_ID, fc::json::to_string( hive::protocol::rc_custom_operation( delegate ) ), post_key );
    };
    rc_delegate( "delegator1", "delegatee", full_vest, delegator1_post_key );
    rc_delegate( "delegator2", "pattern2", full_vest, delegator2_post_key );
    rc_delegate( "delegator3", "pattern3", full_vest, delegator3_post_key );
    generate_block();
    BOOST_REQUIRE_EQUAL( delegatee_rc.rc_manabar.current_mana, pattern2_rc.rc_manabar.current_mana );
    BOOST_REQUIRE_EQUAL( delegatee_rc.rc_manabar.current_mana, pattern3_rc.rc_manabar.current_mana );

    BOOST_TEST_MESSAGE( "Artificially going negative with RC of delegatees to expose problem" );
    //NOTE: the problem was fixed and on top of that we can no longer go negative with mana,
    //so we could probably remove the test, but we keep it as testament of previous bugs
    //int64_t full_rc = db->get_account( "delegatee" ).get_maximum_rc();
    //int64_t min_rc = -1 * ( HIVE_RC_MAX_NEGATIVE_PERCENT * full_rc ) / HIVE_100_PERCENT;
    int64_t min_rc = 0;
    auto rc_set_negative_mana = [&]( const account_object& account )
    {
      db->modify( account, [&]( account_object& acc )
      {
        acc.rc_manabar.current_mana = min_rc;
        acc.rc_manabar.last_update_time = ( db->head_block_time() - fc::seconds( HIVE_RC_REGEN_TIME ) ).sec_since_epoch();
      } );
    };
    rc_set_negative_mana( delegatee_rc );
    rc_set_negative_mana( pattern2_rc );
    rc_set_negative_mana( pattern3_rc );
    generate_block();

    BOOST_TEST_MESSAGE( "Wait for block when comment payout will trigger regen, power down and take away delegation in the same time" );
    generate_blocks( cashout_time - fc::seconds( HIVE_BLOCK_INTERVAL ) );
    withdraw_vesting_operation power_down;
    power_down.account = "delegator1";
    power_down.vesting_shares = full_vest.to_asset();
    push_transaction( power_down, delegator1_private_key );
    auto undelegated = db->get_account( "delegator1" ).get_next_vesting_withdrawal();
    rc_delegate( "delegator3", "pattern3", full_vest - undelegated, delegator3_post_key );
    generate_block();
    //pattern2 RC regeneration used to be triggered by author_reward_operation, but since it doesn't modify RC, that was removed
    //we need to trigger regeneration manually
    db->rc().regenerate_rc_mana( pattern2_rc, db->head_block_time() );
    BOOST_REQUIRE_EQUAL( delegatee_rc.rc_manabar.current_mana, pattern2_rc.rc_manabar.current_mana - undelegated.amount.value );
    //pattern3 undelegated exactly the same amount of RC as was dropped from delegatee by delegator1 powering down
    BOOST_REQUIRE_EQUAL( delegatee_rc.rc_manabar.current_mana, pattern3_rc.rc_manabar.current_mana );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}


BOOST_AUTO_TEST_CASE( update_outdel_overflow_delegatee )
{
  try
  {
    BOOST_TEST_MESSAGE( "Testing: update_outdel_overflow with a vesting delegation" );
    generate_block();
    set_account_creation_fee( HIVE_asset( 300'000 ) ); //increased fee gives accounts a sizable non-delegatable RC adjustment
    generate_block();

    ACTORS( NO_VESTING, (alice)(bob)(dave)(eve)(martin) );
    generate_block();
    const account_object& alice = db->get_account( "alice" );
    const account_object& bob = db->get_account( "bob" );
    const account_object& dave = db->get_account( "dave" );
    const account_object& eve = db->get_account( "eve" );
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    generate_block();
    vest( "alice", HIVE_asset( 10 ) );
    generate_block();

    int64_t vesting_amount = alice.get_vesting().amount.value;
    int64_t creation_rc = alice.get_rc_adjustment().value;

    // We delegate all our vesting shares to bob
    delegate_vesting_shares_operation dvso;
    dvso.vesting_shares = alice.get_vesting().to_asset();
    dvso.delegator = "alice";
    dvso.delegatee = { "bob" };
    push_transaction( dvso, alice_private_key );

    // Delegate 10 rc to dave, the rest to eve, bob has rc_adjustment remaining rc
    delegate_rc_operation op;
    op.from = "bob";
    op.delegatees = { "dave" };
    op.max_rc = 10;
    custom_json_operation custom_op;
    custom_op.required_posting_auths.insert( "bob" );
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    int64_t delegation_rc_cost = 0;
    delegation_rc_cost += push_transaction( custom_op, bob_post_key )->get_rc_cost();
    op.delegatees = { "eve" };
    op.max_rc = vesting_amount - 10 + 1;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    //check that it is not possible to dip into creation_rc, nor will it be accepted at the cost of dropping eve
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, bob_post_key ), "from_delegable_rc >= delta_total" );

    op.max_rc = vesting_amount - 10;
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    delegation_rc_cost += push_transaction( custom_op, bob_post_key )->get_rc_cost();
    generate_block();

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc );

    BOOST_REQUIRE_EQUAL( bob.get_delegated_rc(), uint64_t(vesting_amount) );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob.rc_manabar.current_mana, creation_rc - delegation_rc_cost ); // cost of the two delegate rc ops

    BOOST_REQUIRE_EQUAL( eve.rc_manabar.current_mana, creation_rc + vesting_amount - 10 );
    BOOST_REQUIRE_EQUAL( eve.last_max_rc, creation_rc + vesting_amount - 10 );
    BOOST_REQUIRE_EQUAL( eve.get_received_rc(), uint64_t( vesting_amount - 10 ) );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc + 10 );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), 10 );

    // we remove the vesting delegation and it affects the rc delegations
    dvso.vesting_shares = ASSET( "0.000000 VESTS" );
    push_transaction( dvso, alice_private_key );

    BOOST_REQUIRE_EQUAL( alice.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.get_received_rc(), 0 );
    BOOST_REQUIRE_EQUAL( alice.last_max_rc, creation_rc ); // Still creation_rc because the vesting delegation didn't come back

    BOOST_REQUIRE_EQUAL( bob.get_delegated_rc(), 0 );
    BOOST_REQUIRE_EQUAL( bob.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( bob.get_received_rc(), 0 );

    BOOST_REQUIRE_EQUAL( eve.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( eve.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( eve.get_received_rc(), 0 );

    BOOST_REQUIRE_EQUAL( dave.rc_manabar.current_mana, creation_rc );
    BOOST_REQUIRE_EQUAL( dave.last_max_rc, creation_rc );
    BOOST_REQUIRE_EQUAL( dave.get_received_rc(), 0 );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE( update_outdel_overflow_delegatee_performance )
{
  try
  {
    BOOST_TEST_MESSAGE( "Testing: performance is taken into consideration, when `rc_direct_delegation_object` are removed" );
    generate_block();
    const int removal_limit = 200; //current mainnet value - if we increase it (we should, but it requires HF) we need to change it here as well
    set_account_creation_fee( HIVE_asset( 1 ) ); //it effectively turns off minimum HP delegation limit
    db_plugin->debug_update( [=]( database& db )
    {
      db.set_remove_threshold( removal_limit );
    } );
    generate_block();

    ACTORS( NO_VESTING, (alice)(bob)(carol) );
    generate_block();
    vest( "alice", HIVE_asset( 2'000'000 ) );
    generate_block();

    const uint32_t nr_accounts = removal_limit * 4;

    const account_object& alice_account_initial = db->get_account( "alice" );

    std::vector< performance::initial_data > accounts = performance::generate_accounts( this, nr_accounts );

    auto time_checker = []( std::function<void()> callable, uint32_t value )
    {
      auto _start = std::chrono::high_resolution_clock::now();
      callable();
      auto _end = std::chrono::high_resolution_clock::now();
      double elapsed_time_ms = std::chrono::duration<double, std::milli>( _end - _start ).count();
      BOOST_TEST_MESSAGE( "time: " + std::to_string( elapsed_time_ms ) );
#ifdef NDEBUG
      BOOST_REQUIRE_GT( value, elapsed_time_ms );
#endif
    };

    // We delegate all our vesting shares to bob and carol
    delegate_vesting_shares_operation dvso;
    dvso.vesting_shares = alice_account_initial.get_vesting().to_asset();
    dvso.vesting_shares.amount.value /= 2;
    dvso.delegator = "alice";
    dvso.delegatee = "bob";
    push_transaction( dvso, alice_private_key );
    dvso.delegatee = "carol";
    push_transaction( dvso, alice_private_key );
    generate_block();

    delegate_rc_operation op;
    op.max_rc = 5;
    custom_json_operation custom_op;
    custom_op.id = HIVE_RC_CUSTOM_OPERATION_ID;

    size_t cnt = 0;
    while( cnt < accounts.size() )
    {
      op.delegatees.clear();
      for( size_t i = cnt; i < cnt + HIVE_RC_MAX_ACCOUNTS_PER_DELEGATION_OP; ++i )
        op.delegatees.insert( accounts[i].account );

      cnt += op.delegatees.size();

      BOOST_TEST_MESSAGE( "create delegations" );

      custom_op.required_posting_auths.clear();
      custom_op.required_posting_auths.insert( "bob" );
      op.from = "bob";
      custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
      push_transaction( custom_op, bob_post_key );

      custom_op.required_posting_auths.clear();
      custom_op.required_posting_auths.insert( "carol" );
      op.from = "carol";
      custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
      push_transaction( custom_op, carol_post_key );

      generate_block();
    }

    for( auto& account : accounts )
    {
      const account_object& _account = db->get_account( account.account );
      BOOST_REQUIRE_EQUAL( _account.get_received_rc(), 10 );
    }

    BOOST_TEST_MESSAGE( "remove delegations" );
    dvso.vesting_shares = asset( 0, VESTS_SYMBOL );
    dvso.delegatee = "bob";
    auto push_10 = [this, &dvso, &alice_private_key] { push_transaction( dvso, alice_private_key ); };
    time_checker( push_10, 100 ); // <- removes `removal_limit` delegations from bob, rest (3 limits) postponed
    // leave a bit at carol - we will test supplementation of delegation removal object
    dvso.vesting_shares = asset( op.max_rc * removal_limit * 2, VESTS_SYMBOL );
    dvso.delegatee = "carol";
    auto push_11 = [this, &dvso, &alice_private_key] { push_transaction( dvso, alice_private_key ); };
    time_checker( push_11, 100 ); // <- removes `removal_limit` delegations from carol, rest (1 limit) postponed

    const auto& rc_del_idx = db->get_index<rc_direct_delegation_index, by_id>();
    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 2 * nr_accounts - 2 * removal_limit );

    int i = 0;
    for( auto& account : accounts )
    {
      const account_object& _account = db->get_account( account.account );
      BOOST_REQUIRE_EQUAL( _account.get_received_rc(), ( i >= removal_limit ? 10 : 0 ) );
      ++i;
    }
    generate_block();
    // since automatic expired delegation removal happens on end of block, now there should be twice as many removed,
    // however only the delegations from bob are processed this way (limit on automatic removal is common for all delegations)
    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 2 * nr_accounts - 3 * removal_limit );

    i = 0;
    for( auto& account : accounts )
    {
      const account_object& _account = db->get_account( account.account );
      BOOST_REQUIRE_EQUAL( _account.get_received_rc(), ( i >= removal_limit * 2 ? 10 : i >= removal_limit ? 5 : 0 ) );
      ++i;
    }

    // not possible to create new delegation while previous are still being removed
    op.delegatees.clear();
    op.delegatees.insert( accounts[0].account );
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "bob" );
    op.from = "bob";
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, bob_post_key ), "!_db.is_in_control() || !_db.rc().has_expired_delegation( from_account )" );
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "carol" );
    op.from = "carol";
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, carol_post_key ), "!_db.is_in_control() || !_db.rc().has_expired_delegation( from_account )" );

    // but we can test that dedelegation while previous one did not end just adds to the object
    // (note that execution of dedelegation will in itself stop at removal limit and only excess
    // will be added to the expired delegation object)
    dvso.vesting_shares = asset( 0, VESTS_SYMBOL );
    dvso.delegatee = "carol";
    auto push_12 = [this, &dvso, &alice_private_key] { push_transaction( dvso, alice_private_key ); };
    time_checker( push_12, 100 ); // <- removes `removal_limit` delegations from carol, rest (1 limit) added postponed
    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 2 * nr_accounts - 4 * removal_limit );

    vest( "bob", HIVE_asset( 1'000'000 ) );
    vest( "carol", HIVE_asset( 1'000'000 ) );
    // even though delegators have new vests that could cover not-yet-removed delegations
    // we still continue to remove them (we could make it differently but that complicates
    // implementation - the mechanism is an safety trigger, so it should be simple)
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "bob" );
    op.from = "bob";
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, bob_post_key ), "!_db.is_in_control() || !_db.rc().has_expired_delegation( from_account )" );
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "carol" );
    op.from = "carol";
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, carol_post_key ), "!_db.is_in_control() || !_db.rc().has_expired_delegation( from_account )" );

    generate_block();
    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 2 * nr_accounts - 5 * removal_limit );
    generate_block();

    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 2 * nr_accounts - 6 * removal_limit );
    // bob is free to delegate again, since he was first in queue so his delegations were handled first
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "bob" );
    op.from = "bob";
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    push_transaction( custom_op, bob_post_key );
    // carol is still 2 blocks behind with delegation removal
    custom_op.required_posting_auths.clear();
    custom_op.required_posting_auths.insert( "carol" );
    op.from = "carol";
    custom_op.json = fc::json::to_string( hive::protocol::rc_custom_operation( op ) );
    HIVE_REQUIRE_ASSERT( push_transaction( custom_op, carol_post_key ), "!_db.is_in_control() || !_db.rc().has_expired_delegation( from_account )" );

    generate_block();
    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 2 * nr_accounts - 7 * removal_limit + 1 );
    generate_block();
    BOOST_REQUIRE_EQUAL( rc_del_idx.size(), 1 );
    push_transaction( custom_op, carol_post_key );

    validate_database();
  }
  FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_SUITE_END()

#endif
