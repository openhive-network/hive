    ...
    auto _content = [&is_hf26]( ptr_hardfork_database_fixture& executor )
    {
      ...
      auto _op_comment_comment_options = [&_get_trx]( ptr_hardfork_database_fixture& executor, const fc::ecc::private_key& post_key, bool is_hf26 )
      {
...

        {
          ...
          BOOST_REQUIRE_EQUAL( _comment.get_author_and_permlink_hash(), comment_object::compute_author_and_permlink_hash( executor->get_id( "alice" ), "lemon" ) );
...

        {
          ...
          BOOST_REQUIRE_EQUAL( _comment.get_author_and_permlink_hash(), comment_object::compute_author_and_permlink_hash( executor->get_id( "alice" ), "avocado" ) );
