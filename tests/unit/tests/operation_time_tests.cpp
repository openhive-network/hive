...
BOOST_AUTO_TEST_CASE( account_subsidy_witness_limits )
{
  try
  {
    ...
    do
    {
      ...
      BOOST_CHECK_EQUAL( db->block_reader().fetch_block_by_number( db->head_block_num() )->get_block().transactions.size(), 0u );
      BOOST_CHECK( db->get_account( "alice" ).pending_claimed_accounts == 1 );
    ...
    generate_block();
    BOOST_CHECK_EQUAL( db->block_reader().fetch_block_by_number( db->head_block_num() )->get_block().transactions.size(), 1u );
    BOOST_CHECK( db->get_account( "alice" ).pending_claimed_accounts == 1 );
    ...
    generate_block();
    BOOST_CHECK_EQUAL( db->block_reader().fetch_block_by_number( db->head_block_num() )->get_block().transactions.size(), n );
