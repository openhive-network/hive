...
BOOST_AUTO_TEST_CASE( popped_transactions )
{
  try
  {
    ...
    generate_block();
    check.check_empty();
    BOOST_REQUIRE( get_hive_balance( "alice" ).amount.value == 19990 );
    BOOST_REQUIRE( get_hive_balance( "bob" ).amount.value == 10 );
    BOOST_REQUIRE( get_hive_balance( "carol" ).amount.value == 10000 );
    ...
    PUSH_BLOCK( get_chain_plugin(), block );
    check.check_empty();
