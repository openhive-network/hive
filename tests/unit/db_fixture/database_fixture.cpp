  ...
  )
{
  try
  {
    db_plugin->debug_update( [=]( database& db)
    {
      ...
      const auto& acnt_assets = db.get< assets_object >( assets_object::id_type( acnt.get_id().get_value() ) );
...
bool database_fixture::compare_delayed_vote_count( const account_name_type& name, const std::vector<uint64_t>& data_to_compare )
{
  ...
  const auto* dv = db->find< delayed_votes_object >( delayed_votes_object::id_type( account->get_id().get_value() ) );
  if( dv == nullptr )
    return false;
