...
dhf_database::dhf_database( clean_database_fixture& executor )
            : executor( executor )
{
}

void dhf_database::plugin_prepare()
{
  auto _data_dir = common_init( [&]( appbase::application& app, int argc, char** argv )
  {
    executor.db_plugin = &app.register_plugin< hive::plugins::debug_node::debug_node_plugin >();
    executor.db_plugin->logging = false;
    app.initialize<
      hive::plugins::debug_node::debug_node_plugin
    >( argc, argv );

    executor.db = &app.get_plugin< hive::plugins::chain::chain_plugin >().db();
    BOOST_REQUIRE( executor.db );
  } );

  executor.init_account_pub_key = executor.init_account_priv_key.get_public_key();

  executor.open_database( _data_dir );

  executor.generate_block();
  executor.db->set_hardfork( HIVE_NUM_HARDFORKS );
  executor.generate_block();
...
int64_t dhf_database::create_proposal( std::string creator, std::string receiver,
                  ...
                  asset daily_pay, const fc::ecc::private_key& key, bool with_block_generation )
{
  ...
  const std::string permlink = "permlink" + std::to_string( cnt );
  if( with_block_generation )
    executor.post_comment_with_block_generation(creator, permlink, "title", "body", "test", key);
  else
    executor.post_comment(creator, permlink, "title", "body", "test", key);
  ...
  const auto& proposal_idx = executor.db-> template get_index< proposal_index >().indices(). template get< by_proposal_id >();
  auto itr = proposal_idx.end();
  BOOST_REQUIRE( proposal_idx.begin() != itr );
  --itr;
  BOOST_REQUIRE( creator == itr->creator );
...
void dhf_database::update_proposal(uint64_t proposal_id, std::string creator, asset daily_pay, std::string subject, std::string permlink, const fc::ecc::private_key& key, time_point_sec* end_date)
{
  signed_transaction tx;
  update_proposal_operation op;
...
void dhf_database::vote_proposal( std::string voter, const std::vector< int64_t >& id_proposals, bool approve, const fc::ecc::private_key& key )
{
  update_proposal_votes_operation op;
...
bool dhf_database::exist_proposal( int64_t id )
{
  const auto& proposal_idx = executor.db->get_index< proposal_index >().indices(). template get< by_proposal_id >();
  return proposal_idx.find( id ) != proposal_idx.end();
}

const proposal_object* dhf_database::find_proposal( int64_t id )
{
  const auto& proposal_idx = executor.db->get_index< proposal_index >().indices(). template get< by_proposal_id >();
  auto found = proposal_idx.find( id );
...
void dhf_database::remove_proposal(account_name_type _deleter, flat_set<int64_t> _proposal_id, const fc::ecc::private_key& _key)
{
  remove_proposal_operation rp;
  rp.proposal_owner = _deleter;
  rp.proposal_ids   = _proposal_id;
  ...
  signed_transaction trx;
  trx.operations.push_back( rp );
  trx.set_expiration( executor.db->head_block_time() + HIVE_MAX_TIME_UNTIL_EXPIRATION );
  executor.push_transaction( trx, _key );
  trx.operations.clear();
}

bool dhf_database::find_vote_for_proposal(const std::string& _user, int64_t _proposal_id)
{
  const auto& proposal_vote_idx = executor.db->get_index< proposal_vote_index >().indices(). template get< by_voter_proposal >();
  auto found_vote = proposal_vote_idx.find( boost::make_tuple(_user, _proposal_id ) );
  return found_vote != proposal_vote_idx.end();
}

uint64_t dhf_database::get_nr_blocks_until_maintenance_block()
{
  auto block_time = executor.db->head_block_time();

  auto next_maintenance_time = executor.db->get_dynamic_global_properties().next_maintenance_time;
  auto ret = ( next_maintenance_time - block_time ).to_seconds() / HIVE_BLOCK_INTERVAL;
...
uint64_t dhf_database::get_nr_blocks_until_daily_maintenance_block()
{
  auto block_time = executor.db->head_block_time();

  auto next_maintenance_time = executor.db->get_dynamic_global_properties().next_daily_maintenance_time;
  auto ret = ( next_maintenance_time - block_time ).to_seconds() / HIVE_BLOCK_INTERVAL;
...
void dhf_database::witness_vote( account_name_type _voter, account_name_type _witness, const fc::ecc::private_key& _key, bool _approve )
{
  signed_transaction tx;
  account_witness_vote_operation op;
  op.account = _voter;
...
void dhf_database::proxy( account_name_type _account, account_name_type _proxy, const fc::ecc::private_key& _key )
{
  signed_transaction tx;
  account_witness_proxy_operation op;
  op.account = _account;
