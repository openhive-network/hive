...
{ try {
  ...
  // to allow other tests using condenser_api_fixture with earlier hardforks.
  {
    ...
    const auto create_witness = [&]( const std::string& witness_name )
    {
      ...
      witness_create( witness_name, account_key, witness_name + ".com", witness_key.get_public_key(), HIVE_asset( 1000 ) );
  ...
  for( int i = 0; i < HIVE_MAX_WITNESSES; ++i )
    BOOST_REQUIRE_EQUAL( active_schedule_1.current_shuffled_witnesses[i], scheduled_witnesses_1[i] );
  ...
  for( int i = 0; i < HIVE_MAX_WITNESSES; ++i )
  {
    BOOST_REQUIRE_EQUAL( full_schedule_1.current_shuffled_witnesses[i], all_scheduled_witnesses_1[i] );
    BOOST_REQUIRE_EQUAL( full_schedule_1.future_shuffled_witnesses.value()[i], all_scheduled_witnesses_1[ HIVE_MAX_WITNESSES + 1 + i ] );
  }
  BOOST_REQUIRE_EQUAL( all_scheduled_witnesses_1[ HIVE_MAX_WITNESSES ], "" );
