...
BOOST_AUTO_TEST_CASE( plugin_object_size )
{
  ...
  BOOST_CHECK_EQUAL( sizeof( account_by_key::key_lookup_index::final_node_type ), 120u );
  ...
  BOOST_CHECK_EQUAL( sizeof( account_history_rocksdb::volatile_operation_index::final_node_type ), 176u );
  ...
  BOOST_CHECK_EQUAL( sizeof( market_history::bucket_index::final_node_type ), 160u
#ifdef HIVE_ENABLE_SMT
    + 8
#endif
  );
  ...
  BOOST_CHECK_EQUAL( sizeof( market_history::order_history_index::final_node_type ), 152u );

  BOOST_CHECK_EQUAL( sizeof( rc::rc_resource_param_object ), 368u ); //singleton
  BOOST_CHECK_EQUAL( sizeof( rc::rc_resource_param_index::final_node_type ), 400u );
  BOOST_CHECK_EQUAL( sizeof( rc::rc_pool_object ), 176u ); //singleton
  BOOST_CHECK_EQUAL( sizeof( rc::rc_pool_index::final_node_type ), 208u );
  BOOST_CHECK_EQUAL( sizeof( rc::rc_pending_data ), 88u ); //singleton
  BOOST_CHECK_EQUAL( sizeof( rc::rc_pending_data_index::final_node_type ), 120u );
  BOOST_CHECK_EQUAL( sizeof( rc::rc_account_object ), 80u ); //permanent, as many as account_object, 1.3M atm
  BOOST_CHECK_EQUAL( sizeof( rc::rc_account_index::final_node_type ), 144u );
  BOOST_CHECK_EQUAL( sizeof( rc::rc_usage_bucket_object ), 48u ); //always HIVE_RC_WINDOW_BUCKET_COUNT objects
  BOOST_CHECK_EQUAL( sizeof( rc::rc_usage_bucket_index::final_node_type ), 112u );
  ...
  BOOST_CHECK_EQUAL( sizeof( rc::rc_direct_delegation_index::final_node_type ), 88u );

  BOOST_CHECK_EQUAL( sizeof( reputation::reputation_object ), 32u ); //lasting, as many as account_object, 1.3M atm
  BOOST_CHECK_EQUAL( sizeof( reputation::reputation_index::final_node_type ), 96u );
  ...
  BOOST_CHECK_EQUAL( sizeof( transaction_status::transaction_status_index::final_node_type ), 128u );
  ...
  BOOST_CHECK_EQUAL( sizeof( witness::witness_custom_op_index::final_node_type ), 96u );
}
