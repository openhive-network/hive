...
"$SRC_DIR/scripts/ci-helpers/prepare_data_and_shm_dir.sh" --data-base-dir="$DATA_DIR"
...
source "${SRC_DIR}"/scripts/run_hived_img.sh "$HIVED_IMAGE_TAG" --name=hived-snapshot-instance --data-dir="$DATA_DIR" --load-snapshot "$SNAPSHOT_NAME"

# "${SRC_DIR}"/build_mainnet/programs/hived/hived --data-dir "$DATA_DIR" --load-snapshot "$SNAPSHOT_NAME" --snapshot-root-dir="$DATA_DIR/snapshot"
