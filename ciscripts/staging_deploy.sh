...
echo $ID > ID.txt
#screen -L -Logfile deploy.log -dmS hived-deploy-$CI_ENVIRONMENT_NAME-$ID 
$CI_PROJECT_DIR/sync/$CI_ENVIRONMENT_NAME/hived --replay-blockchain --set-benchmark-interval 100000 --stop-replay-at-block $STOP_REPLAY_AT --webserver-http-endpoint 127.0.0.1:$HTTPS_PORT --webserver-ws-endpoint 127.0.0.1:$WS_PORT --exit-after-replay --advanced-benchmark --dump-memory-details  -d $CI_PROJECT_DIR/sync/$CI_ENVIRONMENT_NAME --shared-file-dir $CI_PROJECT_DIR/sync/$CI_ENVIRONMENT_NAME
